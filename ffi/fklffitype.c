...
size_t _struct_hashFunc(void* key,FklHashTable* table)
{
	FklSid_t sid=*(FklSid_t*)key;
	return sid%table->size;
}

void _struct_freeItem(void* item)
{
	free(item);
}

int _struct_keyEqual(void* pkey0,void* pkey1)
{
...
{
	.__hashFunc=_struct_hashFunc,
	.__freeItem=_struct_freeItem,
	.__keyEqual=_struct_keyEqual,
	.__getKey=_struct_getKey,
};
...
FklTypeId_t fklFfiNewStructType(FklSid_t structName,uint32_t num,FklSid_t symbols[],FklTypeId_t memberTypes[])
{
	...
	if(!id)
	{
		...
		for(uint32_t i=0;i<num;i++)
		{
			...
			fklInsNrptHashItem(newStructHashItem(symbols[i],memberTypes[i],offset),tmp->layout);
...
FklTypeId_t fklFfiNewUnionType(FklSid_t unionName,uint32_t num,FklSid_t symbols[],FklTypeId_t memberTypes[])
{
	...
	tmp->layout=fklNewHashTable(8,0.75,&FfiMemberHashMethodTable);
	for(uint32_t i=0;i<num;i++)
		fklInsNrptHashItem(newStructHashItem(symbols[i],memberTypes[i],0),tmp->layout);
...
static void initStructTypeId(FklTypeId_t id,FklSid_t structNameId,uint32_t num,FklSid_t* symbols,FklTypeId_t* memberTypes)
{
	...
	for(uint32_t i=0;i<num;i++)
	{
		...
		size_t align=fklFfiGetTypeAlign(tu);
		offset+=(offset%align)?align-offset%align:0;
		fklInsNrptHashItem(newStructHashItem(symbols[i],memberTypes[i],offset),ost->layout);
		size_t memberSize=fklFfiGetTypeSize(tu);
		offset+=memberSize;
	}
...
static void initUnionTypeId(FklTypeId_t id,FklSid_t unionNameId,uint32_t num,FklSid_t* symbols,FklTypeId_t* memberTypes)
{
	...
	for(uint32_t i=0;i<num;i++)
		fklInsNrptHashItem(newStructHashItem(symbols[i],memberTypes[i],0),out->layout);
...
{
	return type>0&&type<=FKL_FFI_VPTR;
}

int fklFfiIsIntegerTypeId(FklTypeId_t type)
{
	return (type>=FKL_FFI_SHORT&&type<FKL_FFI_FLOAT)
		||(type>=FKL_FFI_INT8_T&&type<FKL_FFI_VPTR);
}
...
{
	return type==FKL_FFI_FLOAT||type==FKL_FFI_DOUBLE;
}
...
{
	FklDefTypeUnion tu=fklFfiGetTypeUnion(type);
	return type==FKL_FFI_VPTR||FKL_GET_TYPES_TAG(tu.all)==FKL_DEF_PTR_TYPE_TAG;
}
