...
static inline FklVMvalue* recover_error_scene(FklVM* exe,struct FuvErrorRecoverData* rd)
{
	FklVMvalue** cur=rd->stack_values;
	FklVMvalue** end=&cur[rd->stack_values_num];
	for(;cur<end;cur++)
...
static void error_check_idle_close_cb(uv_handle_t* h)
{
	...
	FklVMvalue* ev=recover_error_scene(exe,&fuv_loop->data.error_recover_data);
