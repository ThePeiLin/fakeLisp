...
uint32_t fklRegexMatchpInCstr(const FklRegexCode* pattern,const char* text,uint32_t* ppos);
uint32_t fklRegexMatchpInCharBuf(const FklRegexCode* pattern,const char* text,uint32_t len,uint32_t* ppos);
...
void fklRegexPrintAsC(const FklRegexCode*
		,const char* prefix
		,const char* pattern
...
const FklRegexCode* fklAddRegexStr(FklRegexTable* table
		,const FklString* str);

const FklRegexCode* fklAddRegexCharBuf(FklRegexTable* table
		,const char* buf
		,size_t len);
