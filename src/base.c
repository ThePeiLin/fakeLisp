...
void fklPushPtrStack(void *data, FklPtrStack *stack) {
    if (stack->top == stack->size) {
        void **tmpData = (void **)fklRealloc(
            stack->base, (stack->size + stack->inc) * sizeof(void *));
        FKL_ASSERT(tmpData);
        stack->base = tmpData;
...
void fklUninitPtrStack(FklPtrStack *s) { free(s->base); }
...
void fklDestroyQueueNode(FklQueueNode *tmp) { free(tmp); }
...
int fklIsPtrQueueEmpty(FklPtrQueue *queue) { return queue->head == NULL; }
...
void fklPushIntStack(int64_t data, FklIntStack *stack) {
    if (stack->top == stack->size) {
        int64_t *tmpData = (int64_t *)fklRealloc(
            stack->base, (stack->size + stack->inc) * sizeof(int64_t));
        FKL_ASSERT(tmpData);
        stack->base = tmpData;
        stack->size += stack->inc;
    }
...
int fklIsUintStackEmpty(FklUintStack *stack) { return stack->top == 0; }
...
void fklUninitUintStack(FklUintStack *r) { free(r->base); }
...
void fklPushUintStack(uint64_t data, FklUintStack *stack) {
    if (stack->top == stack->size) {
        uint64_t *tmpData = (uint64_t *)fklRealloc(
            stack->base, (stack->size + stack->inc) * sizeof(uint64_t));
        FKL_ASSERT(tmpData);
        stack->base = tmpData;
        stack->size += stack->inc;
    }
...
void fklRecycleUintStack(FklUintStack *stack) {
    if (stack->size - stack->top > stack->inc) {
        ...
        uint64_t *tmpData =
            (uint64_t *)fklRealloc(stack->base, size * sizeof(uint64_t));
...
void fklUninitU8Stack(FklU8Stack *r) { free(r->base); }
...
void fklPushU8Stack(uint8_t data, FklU8Stack *stack) {
    if (stack->top == stack->size) {
        uint8_t *tmpData = (uint8_t *)fklRealloc(
            stack->base, (stack->size + stack->inc) * sizeof(uint8_t));
        FKL_ASSERT(tmpData);
        stack->base = tmpData;
        stack->size += stack->inc;
    }
...
int fklIsU8StackEmpty(FklU8Stack *s) { return s->top == 0; }

void fklRecycleU8Stack(FklU8Stack *s) {
    if (s->size - s->top > s->inc) {
        uint32_t size = s->size - s->inc;
        uint8_t *tmpData =
            (uint8_t *)fklRealloc(s->base, size * sizeof(uint8_t));
        FKL_ASSERT(tmpData || !size);
        s->base = tmpData;
        s->size -= s->inc;
    }
