...
static void info_work_cb(FklCodegenInfo *info, void *ctx) {
    if (!info->is_macro) {
        ...
        fklSidUsetPut2(&dctx->file_sid_set, info->fid);
    }
}
...
static void B_int33(FKL_VM_INS_FUNC_ARGL) {
    ...
    fklVMexecuteInstruction(
        exe, get_breakpoint_with_ins(exe->debug_ctx, ins)->origin_op, ins,
        exe->top_frame);
}
