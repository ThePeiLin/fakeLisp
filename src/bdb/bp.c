...
static inline void mark_breakpoint_deleted(Breakpoint* bp,BreakpointTable* bt)
{
...
{
	for(FklHashTableItem* l=bt->idx_ht.first
			;l
			;l=l->next)
	{
...
{
	BreakpointIdxHashItem item={.idx=0,.bp=NULL,};
	BreakpointTable* bt=&dctx->bt;
...
{
	BreakpointTable* bt=&dctx->bt;
	for(FklHashTableItem* list=bt->idx_ht.first
			;list
			;list=list->next)
	{
	...
	for(Breakpoint* bp=bt->deleted_breakpoints
			;bp
			;bp=bp->next)
	{
		if(bp->cond_exp_obj)
...
BreakpointInsHashItem* getBreakpointHashItem(DebugCtx* dctx,const FklInstruction* ins)
{
