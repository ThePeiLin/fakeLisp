...
static void strbuf_as_princ(const FklVMud* ud,FklStringBuffer* buf,FklVMgc* gc)
{
	FKL_DECL_UD_DATA(bufa,FklStringBuffer,ud);
...
static inline FklVMvalue* create_vmstrbuf(FklVM* exe,size_t capacity,FklVMvalue* rel)
{
...
static inline FklVMvalue* create_vmstrbuf2(FklVM* exe,size_t size,const char* ptr,FklVMvalue* rel)
{
	FklVMvalue* r=fklCreateVMvalueUd(exe,&StringBufferMetaTable,rel);
	FKL_DECL_VM_UD_DATA(buf,FklStringBuffer,r);
	fklInitStringBuffer(buf);
...
static FklVMvalue* strbuf_copy_append(FklVM* exe,const FklVMud* ud,uint32_t argc,FklVMvalue* const* top)
{
	FKL_DECL_UD_DATA(buf,FklStringBuffer,ud);
...
static int export_make_strbuf(FKL_CPROC_ARGL)
{
	...
	{
		fklResBp(exe);
		FKL_VM_PUSH_VALUE(exe,create_vmstrbuf(exe,0,FKL_VM_CPROC(ctx->proc)->dll));
		return 0;
	}
...
static int export_make_strbuf_with_capacity(FKL_CPROC_ARGL)
{
	...
	if(fklIsVMnumberLt0(size))
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_NUMBER_SHOULD_NOT_BE_LT_0,exe);
	size_t len=fklVMgetUint(size);
	FKL_VM_PUSH_VALUE(exe,create_vmstrbuf(exe,len,FKL_VM_CPROC(ctx->proc)->dll));
	return 0;
}
...
static int export_strbuf(FKL_CPROC_ARGL)
{
	...
	if(size==1)
	{
		...
		if(FKL_IS_CHR(str_or_buf))
		{
			...
			FKL_VM_PUSH_VALUE(exe,create_vmstrbuf2(exe,1,&c,FKL_VM_CPROC(ctx->proc)->dll));
		}
		else if(FKL_IS_STR(str_or_buf))
		{
			const FklString* b=FKL_VM_STR(str_or_buf);
			FKL_VM_PUSH_VALUE(exe,create_vmstrbuf2(exe,b->size,b->str,FKL_VM_CPROC(ctx->proc)->dll));
		}
		else if(is_strbuf_ud(str_or_buf))
		{
			FKL_DECL_VM_UD_DATA(b,FklStringBuffer,str_or_buf);
			FKL_VM_PUSH_VALUE(exe,create_vmstrbuf2(exe,b->index,b->buf,FKL_VM_CPROC(ctx->proc)->dll));
		}
		else
...
static int export_strbuf_to_bytevector(FKL_CPROC_ARGL)
{
	...
	FKL_CHECK_REST_ARG(exe);
	FKL_DECL_VM_UD_DATA(b,FklStringBuffer,buf);
	FKL_VM_PUSH_VALUE(exe,fklCreateVMvalueBvec2(exe,b->index,(const uint8_t*)b->buf));
	return 0;
}
...
static int export_strbuf_to_symbol(FKL_CPROC_ARGL)
{
	...
	FKL_CHECK_TYPE(obj,is_strbuf_ud,exe);
	FKL_DECL_VM_UD_DATA(buf,FklStringBuffer,obj);
	FKL_VM_PUSH_VALUE(exe,FKL_MAKE_VM_SYM(fklVMaddSymbolCharBuf(exe->gc,buf->buf,buf->index)->id));
	return 0;
}
...
static int export_strbuf_set1(FKL_CPROC_ARGL)
{
	...
	if(!FKL_IS_CHR(target)&&!fklIsVMint(target))
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INCORRECT_TYPE_VALUE,exe);
	buf->buf[index]=FKL_IS_CHR(target)?FKL_GET_CHR(target):fklVMgetInt(target);
...
static int export_strbuf_fmt(FKL_CPROC_ARGL)
{
	...
	if(!is_strbuf_ud(buf_obj)||(!FKL_IS_STR(fmt_obj)&&!is_strbuf_ud(fmt_obj)))
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INCORRECT_TYPE_VALUE,exe);
	...
	else
	{
		...
		err_type=fklVMformat2(exe,buf,fmt_buf->buf,&fmt_buf->buf[fmt_buf->index],&len);
	}
...
static int export_substrbuf(FKL_CPROC_ARGL)
{
	...
	if(start>size||end<start||end>size)
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INVALIDACCESS,exe);
	size=end-start;
	FklVMvalue* r=create_vmstrbuf2(exe,size,buf->buf+start,FKL_VM_UD(ostr)->rel);
	FKL_VM_PUSH_VALUE(exe,r);
	return 0;
}
...
static int export_sub_strbuf(FKL_CPROC_ARGL)
{
	...
	if(start+osize>size)
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INVALIDACCESS,exe);
	FklVMvalue* r=create_vmstrbuf2(exe,osize,buf->buf+start,FKL_VM_UD(ostr)->rel);
	FKL_VM_PUSH_VALUE(exe,r);
	return 0;
}
