...
static int isValidSyntaxPattern(const FklVMvalue *p) {
    ...
    FklSidTable symbolTable;
    fklSidTableInit(&symbolTable);
    ...
    while (!fklVMvalueVectorIsEmpty(&exe)) {
        ...
        if (slotV) {
            ...
            if (fklSidTableHas2(&symbolTable, sid)) {
                fklSidTableUninit(&symbolTable);
                fklVMvalueVectorUninit(&exe);
                return 0;
            }
            fklSidTablePut2(&symbolTable, sid);
        }
        if (FKL_IS_PAIR(c)) {
...
#define AND_OR_MAP_PATTERN(DEFAULT, EXIT)                                      \
    switch (ctx->context) {                                                    \
    case 0: {                                                                  \
        FKL_DECL_AND_CHECK_ARG(proc, exe);                                     \
        FKL_CHECK_TYPE(proc, fklIsCallable, exe);                              \
    ...
    case 1: {                                                                  \
        ...
        if (base[0] == FKL_VM_NIL)                                             \
            ...
            FklVMvalue *proc = FKL_VM_GET_VALUE(exe, 1);                       \
            fklSetBp(exe);                                                     \
            for (FklVMvalue *const *const end = &base[arg_num - 2];            \
                 base < end; base++) {                                         \
                FKL_VM_PUSH_VALUE(exe, FKL_VM_CAR(*base));                     \
                *base = FKL_VM_CDR(*base);                                     \
            }                                                                  \
...
static int builtin_foreach(FKL_CPROC_ARGL) { AND_OR_MAP_PATTERN(FKL_VM_NIL, ); }

static int builtin_andmap(FKL_CPROC_ARGL) {
    AND_OR_MAP_PATTERN(
        FKL_VM_TRUE, if (r == FKL_VM_NIL) {
            FKL_VM_SET_TP_AND_PUSH_VALUE(exe, ctx->rtp, r);
            return 0;
        });
}
