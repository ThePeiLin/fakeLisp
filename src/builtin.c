...
static alignas(8) FklVMvalueFp StdinUserDataValue;

static alignas(8) FklVMvalueFp StdoutUserDataValue;

static alignas(8) FklVMvalueFp StderrUserDataValue;
...
static int builtin_copy(FKL_CPROC_ARGL) {
    FKL_CPROC_CHECK_ARG_NUM(exe, argc, 1);
    FklVMvalue *retval = fklCopyVMvalue(exe, FKL_CPROC_GET_ARG(exe, ctx, 0));
    if (!retval)
...
static int builtin_append(FKL_CPROC_ARGL) {
    ...
    FKL_ASSERT(argc > 1);
    FklVMvalue *r = fklAppendVMvalue(exe,
            obj,
            argc - 1,
            &FKL_CPROC_GET_ARG(exe, ctx, 1));
    if (r == NULL)
...
static int builtin_append1(FKL_CPROC_ARGL) {
    ...
    FKL_ASSERT(argc > 1);
    FklVMvalue *r = fklAppendVMvalue1(exe,
            obj,
            argc - 1,
            &FKL_CPROC_GET_ARG(exe, ctx, 1));
...
static int builtin_subvector(FKL_CPROC_ARGL) {
    ...
    if (fklIsVMnumberLt0(vstart) || fklIsVMnumberLt0(vend))
        FKL_RAISE_BUILTIN_ERROR(FKL_ERR_NUMBER_SHOULD_NOT_BE_LT_0, exe);
    FklVMvalueVec *vec = FKL_VM_VEC(ovec);
    ...
    if (start > size || end < start || end > size)
        FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INVALIDACCESS, exe);
    size = end - start;
    FklVMvalue *r = fklCreateVMvalueVec2(exe, size, vec->base + start);
    FKL_CPROC_RETURN(exe, ctx, r);
    return 0;
}
...
static inline double bigint_to_double(const FklVMvalueBigInt *b) {
    const FklBigInt bi = fklVMbigIntToBigInt(b);
...
static inline void init_custom_read_frame(FklVM *exe,
    ...
    initReadCtx(f->data, name, exe, vfp, parser);
}

static inline void
initFrameToReadFrame(FklVM *exe, const char *name, FklVMvalue *vfp) {
    FklVMframe *f = exe->top_frame;
    f->type = FKL_FRAME_OTHEROBJ;
    f->t = &ReadContextMethodTable;
    initReadCtx(f->data, name, exe, vfp, FKL_VM_NIL);
}
...
static inline FklGrammerIgnore *vm_vec_to_ignore(FklVMvalueVec *vec,
        ValueToGrammerSymArgs *args,
...
static int custom_parse_frame_step(void *d, FklVM *exe) {
    CustomParseCtx *parse_ctx = (CustomParseCtx *)d;
    struct ParseCtx *pctx = parse_ctx->pctx;
    if (parse_ctx->state == PARSE_REDUCING) {
        FklVMvalue *ast = FKL_VM_POP_TOP_VALUE(exe);
        fklInitNontermAnalysisSymbol(
                fklAnalysisSymbolVectorPushBack(&pctx->symbolStack, NULL),
                pctx->reducing_sid,
    ...
    if (accept) {
        ...
        if (parse_ctx->box) {
            ...
            if (offset > FKL_FIX_INT_MAX)
                FKL_VM_BOX(parse_ctx->box) =
                        fklCreateVMvalueBigIntWithU64(exe, offset);
            else
                FKL_VM_BOX(parse_ctx->box) = FKL_MAKE_VM_FIX(offset);
        }
        return 0;
    } else
...
static inline void init_custom_parse_frame(FklVM *exe,
    ...
    init_custom_parse_ctx(f->data, name, parser, str, box);
}
...
#define GET_STDIN() ((FklVMvalue *)&StdinUserDataValue)
#define GET_STDOUT() ((FklVMvalue *)&StdoutUserDataValue)
#define GET_STDERR() ((FklVMvalue *)&StderrUserDataValue)

#define GET_SEEK_SET() (exe->gc->seek_set)

#define GET_SEEK_CUR() (exe->gc->seek_cur)

#define GET_SEEK_END() (exe->gc->seek_end)
...
static int builtin_read(FKL_CPROC_ARGL) {
    ...
    if (!stream || fklIsVMvalueFp(stream)) {
        ...
        if (parser)
            ...
            initFrameToReadFrame(exe, FKL_VM_CPROC(ctx->proc)->name, fpv);
    }
    return 1;
}
...
static int builtin_fgetc(FKL_CPROC_ARGL) {
    ...
    FklVMvalue *stream = argc ? FKL_CPROC_GET_ARG(exe, ctx, 0) : GET_STDIN();
...
static int builtin_fgetd(FKL_CPROC_ARGL) {
    ...
    FklVMvalue *file = argc > 1 ? FKL_CPROC_GET_ARG(exe, ctx, 1) : GET_STDIN();
    int d = EOF;
    if (del) {
...
static int builtin_prin1(FKL_CPROC_ARGL) {
    ...
    FklVMvalue *obj = FKL_CPROC_GET_ARG(exe, ctx, 0);
    FklVMvalue *file = argc > 1 ? FKL_CPROC_GET_ARG(exe, ctx, 1) : GET_STDOUT();
    FKL_CHECK_TYPE(file, fklIsVMvalueFp, exe);
...
static int builtin_newline(FKL_CPROC_ARGL) {
    FKL_CPROC_CHECK_ARG_NUM2(exe, argc, 0, 1);
    FklVMvalue *file = argc ? FKL_CPROC_GET_ARG(exe, ctx, 0) : GET_STDOUT();
    FKL_CHECK_TYPE(file, fklIsVMvalueFp, exe);
    CHECK_FP_OPEN(file, exe);
    CHECK_FP_WRITABLE(file, exe);
    fputc('\n', FKL_VM_FP(file)->fp);
...
static inline int match_pattern(const FklVMvalue *pattern,
    ...
    fklPairVectorPushBack2(&s,
            (FklPair){ .car = FKL_VM_CAR(FKL_VM_CDR(pattern)), .cdr = exp });
    int r = 0;
    while (!fklPairVectorIsEmpty(&s)) {
        const FklPair *p = fklPairVectorPopBackNonNull(&s);
        FklVMvalue *v0 = p->car;
        FklVMvalue *v1 = p->cdr;
        ...
        if (slotV)
            ...
            fklPairVectorPushBack2(&s,
                    (FklPair){ .car = FKL_VM_BOX(v0), .cdr = FKL_VM_BOX(v1) });
            ...
            for (; c0 > b0; --c0, --c1) {
                fklPairVectorPushBack2(&s, (FklPair){ .car = *c0, .cdr = *c1 });
            }
        } else if (FKL_IS_HASHTABLE(v0) && FKL_IS_HASHTABLE(v1)) {
            FklVMvalueHash *h0 = FKL_VM_HASH(v0);
            FklVMvalueHash *h1 = FKL_VM_HASH(v1);
            r = h0->eq_type != h1->eq_type || h0->ht.count != h1->ht.count;
            if (r)
                break;
            FklValueHashMapNode *i0 = h0->ht.last;
            FklValueHashMapNode *i1 = h1->ht.last;
            while (h0) {
                fklPairVectorPushBack2(&s,
                        (FklPair){ .car = i0->v, .cdr = i1->v });
                fklPairVectorPushBack2(&s,
                        (FklPair){ .car = i0->k, .cdr = i1->k });
                i0 = i0->prev;
                i1 = i1->prev;
            }
...
static int isValidSyntaxPattern(const FklVMvalue *p) {
    ...
    FklValueHashSet symbol_table;
    fklValueHashSetInit(&symbol_table);
    FklValueVector exe;
    fklValueVectorInit(&exe, 32);
    fklValueVectorPushBack2(&exe, FKL_TYPE_CAST(FklVMvalue *, body));
    while (!fklValueVectorIsEmpty(&exe)) {
        const FklVMvalue *c = *fklValueVectorPopBackNonNull(&exe);
        FklVMvalue *slotV = isSlot(head, c);
        if (slotV) {
            FklVMvalue *sid = FKL_GET_SYM(slotV);
            if (fklValueHashSetHas2(&symbol_table, sid)) {
                fklValueHashSetUninit(&symbol_table);
                fklValueVectorUninit(&exe);
                return 0;
            }
            fklValueHashSetPut2(&symbol_table, sid);
        }
        if (FKL_IS_PAIR(c)) {
            fklValueVectorPushBack2(&exe, FKL_VM_CAR(c));
            ...
            size_t size = vec->size;
            for (size_t i = 0; i < size; i++)
                fklValueVectorPushBack2(&exe, base[i]);
        } else if (FKL_IS_HASHTABLE(c)) {
            for (FklValueHashMapNode *h = FKL_VM_HASH(c)->ht.first; h;
                    h = h->next) {
                fklValueVectorPushBack2(&exe, h->k);
                fklValueVectorPushBack2(&exe, h->v);
            }
        }
    }
    fklValueHashSetUninit(&symbol_table);
...
static int builtin_pmatch(FKL_CPROC_ARGL) {
    ...
    FklVMvalueHash *hash = FKL_VM_HASH(r);
...
static void error_handler_frame_atomic(void *data, FklVMgc *gc) {
    ...
    FklPair *pairs = c->err_handlers;
    FklPair *const end = pairs + c->num;
    for (; pairs < end; ++pairs) {
...
errorCallBackWithErrorHandler(FklVMframe *f, FklVMvalue *errValue, FklVM *exe) {
    ...
    FklPair *err_handlers = c->err_handlers;
    FklPair *const end = err_handlers + c->num;
...
#define GET_PROC (1)
    ...
    fklPairVectorInit(&err_handlers, argc);
    FklPair *pair = NULL;
    int state = GET_LIST;
    FklVMvalue **arg_base = &FKL_CPROC_GET_ARG(exe, ctx, 0);
    ...
    for (++arg_base; arg_base < end; ++arg_base) {
        ...
        case GET_LIST:
            if (!is_symbol_list(v)) {
                fklPairVectorUninit(&err_handlers);
                FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INCORRECT_TYPE_VALUE, exe);
            }
            pair = fklPairVectorPushBack(&err_handlers, NULL);
            pair->car = v;
            break;
        case GET_PROC:
            if (!fklIsCallable(v)) {
...
static int builtin_hash(FKL_CPROC_ARGL) {
    FklVMvalue *r = fklCreateVMvalueHashEq(exe);
    FklVMvalueHash *ht = FKL_VM_HASH(r);
    FklVMvalue **base = &FKL_CPROC_GET_ARG(exe, ctx, 0);
    FklVMvalue **const end = base + argc;
    for (; base < end; ++base) {
        FklVMvalue *cur = *base;
...
static int builtin_hash_ref(FKL_CPROC_ARGL) {
    ...
    FklValueHashMapElm *r = fklVMhashTableGet(FKL_VM_HASH(ht), key);
    if (r)
...
static int builtin_hash_ref4(FKL_CPROC_ARGL) {
    ...
    FklVMvalue *key = FKL_CPROC_GET_ARG(exe, ctx, 1);
    FKL_CHECK_TYPE(ht, FKL_IS_HASHTABLE, exe);
    FklValueHashMapElm *i = fklVMhashTableGet(FKL_VM_HASH(ht), key);
    if (i)
...
static int builtin_hash_ref1(FKL_CPROC_ARGL) {
    ...
    FKL_CHECK_TYPE(ht, FKL_IS_HASHTABLE, exe);
    FklValueHashMapElm *item = fklVMhashTableRef1(FKL_VM_HASH(ht), key, toSet);
...
static int builtin_hash_clear(FKL_CPROC_ARGL) {
    ...
    FklVMvalue *ht = FKL_CPROC_GET_ARG(exe, ctx, 0);
    FKL_CHECK_TYPE(ht, FKL_IS_HASHTABLE, exe);
    fklValueHashMapClear(&FKL_VM_HASH(ht)->ht);
...
static int builtin_hash_set8(FKL_CPROC_ARGL) {
    ...
    FklVMvalueHash *hash = FKL_VM_HASH(ht);
...
static inline FklVMvalue *inlfunc_box0(INL_FUNC_ARGS) {
    return fklCreateVMvalueCodeObjExt(exe,
            FKL_MAKE_INS_IsA(FKL_OP_PUSH_BOX, 0),
            fid,
            line,
            scope);
}
...
static FklVMvalue *inlfunc_ret0(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_DROP, FKL_SUBOP_DROP_ALL);
    FklVMvalue *r = inl_0_arg_func2(exe, ins, fid, line, scope);
    ins = FKL_MAKE_INS_I(FKL_OP_PUSH_NIL);
    fklByteCodeLntPushBackIns(FKL_VM_CO(r), ins, fid, line, scope);
    ins = FKL_MAKE_INS_I(FKL_OP_RET);
    fklByteCodeLntPushBackIns(FKL_VM_CO(r), ins, fid, line, scope);
    return r;
}
...
static FklVMvalue *inlfunc_car(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_PAIR, FKL_SUBOP_PAIR_CAR);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_cdr(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_PAIR, FKL_SUBOP_PAIR_CDR);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_vec_first(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_VEC, FKL_SUBOP_VEC_FIRST);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_vec_last(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_VEC, FKL_SUBOP_VEC_LAST);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_add_1(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_ADDK, 1);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}
...
static FklVMvalue *inlfunc_sub1(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_SUB, 1);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_div1(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_DIV, 1);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_add1(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_ADD, 1);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_mul1(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_MUL, 1);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_box(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_PUSH_BOX, 1);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_unbox(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_BOX, FKL_SUBOP_BOX_UNBOX);
    return inl_1_arg_func2(ins, bcs, fid, line, scope);
}
...
static FklVMvalue *inlfunc_nth(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_PAIR, FKL_SUBOP_PAIR_NTH);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_vec_ref(INL_FUNC_ARGS) {
    FklByteCodelnt *ib = FKL_VM_CO(bcs[1]);
    if (ib->bc.len == 1 && FKL_INS_OP(ib->bc.code[0]) == FKL_OP_PUSH_0) {
        FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_VEC, FKL_SUBOP_VEC_FIRST);
        return inl_1_arg_func2(ins, bcs, fid, line, scope);
    } else {
        FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_VEC, FKL_SUBOP_VEC_REF);
        return inl_2_arg_func2(ins, bcs, fid, line, scope);
    }
}

static FklVMvalue *inlfunc_str_ref(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_STR, FKL_SUBOP_STR_REF);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}
...
static FklVMvalue *inlfunc_car_set(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_PAIR, FKL_SUBOP_PAIR_CAR_SET);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_cdr_set(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_PAIR, FKL_SUBOP_PAIR_CDR_SET);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_box_set(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_BOX, FKL_SUBOP_BOX_SET);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_hash_ref_2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_HASH, FKL_SUBOP_HASH_REF_2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_add2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_ADD, 2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_sub2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_SUB, 2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_gt2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_GT, 2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_lt2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_LT, 2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_ge2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_GE, 2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_le2(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_LE, 2);
    return inl_2_arg_func2(ins, bcs, fid, line, scope);
}
...
static FklVMvalue *inlfunc_eqn3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_EQN, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_gt3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_GT, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_lt3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_LT, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_ge3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_GE, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_le3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_LE, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_add3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_ADD, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_sub3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_SUB, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_mul3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_MUL, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_div3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_DIV, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_idiv3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_IDIV, 3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_vec_set(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_VEC, FKL_SUBOP_VEC_SET);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_str_set(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_STR, FKL_SUBOP_STR_SET);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}
...
static FklVMvalue *inlfunc_hash_ref_3(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_HASH, FKL_SUBOP_HASH_REF_3);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}

static FklVMvalue *inlfunc_hash_set(INL_FUNC_ARGS) {
    FklIns ins = FKL_MAKE_INS_IsA(FKL_OP_HASH, FKL_SUBOP_HASH_SET);
    return inl_3_arg_func2(ins, bcs, fid, line, scope);
}
