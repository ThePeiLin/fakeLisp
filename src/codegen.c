    ...
    if (lib->named_prod_groups.t) {
        ...
        for (FklHashTableItem *prod_group_item = lib->named_prod_groups.first;
            ...
            fklStringBufferConcatWithString(
                &buffer, fklGetSymbolWithId(group->id, pst)->k);
...
static void *custom_action(void *c, void *outerCtx, void *nodes[], size_t num,
    ...
    fklPmatchTableAdd2(&ht, fklAddSymbolCstr("$$", ctx->pst)->v, nodes_vector);
    fklPmatchTableAdd2(&ht, fklAddSymbolCstr("line", ctx->pst)->v, line_node);
...
static inline FklGrammerProduction *nast_vector_to_production(
    ...
    if (action_type == codegen->outer_ctx->builtInPatternVar_builtin) {
        ...
        fklSidTablePut2(&symbolSet, fklAddSymbolCstr("$$", pst)->v);
