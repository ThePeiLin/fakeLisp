...
inline static void process_unresolve_ref(FklCodegenEnv* env,FklPrototypePool* cp)
{
	...
	while(!fklIsPtrStackEmpty(urefs))
	{
		...
		{
			ref->cidx=fklAddCodegenDefBySid(uref->id,env);
			ref->isLocal=1;
		}
		else
...
static FklUnReSymbolRef* createUnReSymbolRef(FklSid_t id,uint32_t idx,uint32_t prototypeId)
{
		...
		if(cur)
		{
			...
			else
			{
				...
				else
				{
					...
					FklUnReSymbolRef* unref=createUnReSymbolRef(id,idx,env->prototypeId);
...
static FklByteCodelnt* processArgs(const FklNastNode* args,FklCodegenEnv* curEnv,FklCodegen* codegen)
{
	...
	for(;args->type==FKL_NAST_PAIR;args=args->u.pair->cdr)
	{
		...
		if(fklIsSymbolDefined(cur->u.sym,curEnv->cscope,curEnv))
		{
			fklDestroyByteCodelnt(retval);
			return NULL;
		}
	...
	{
		if(fklIsSymbolDefined(args->u.sym,curEnv->cscope,curEnv))
		{
			fklDestroyByteCodelnt(retval);
			return NULL;
		}
...
static CODEGEN_FUNC(codegen_setq)
{
	...
	FklPtrStack* stack=fklCreatePtrStack(2,1);
	fklPushPtrQueue(fklMakeNastNodeRef(value),queue);
	if(fklIsSymbolDefined(name->u.sym,curEnv->cscope,curEnv))
	{
		uint32_t idx=fklAddCodegenDefBySid(name->u.sym,curEnv);
		fklPushPtrStack(makeDupAndPutLoc(name,idx,codegen),stack);
	}
...
BC_PROCESS(_library_export_macro_with_prefix_pc_process)
{
	FklPtrStack* stack=GET_STACK(context);
		...
		,FklPtrStack* codegenQuestStack)
{
	...
	if(!libId)
	{
		...
		add_symbol_to_locale_env_in_list(export
				,curEnv
				,codegen->globalSymTable
				,codegen->publicSymbolTable
				,&idStack
		...
		,FklPtrStack* codegenQuestStack)
{
	...
	if(!libId)
	{
		...
		FklCodegenQuest* prevQuest=createCodegenQuest(_library_be_imported_with_prefix_bc_process
				,createCodegenQuestContext(bcStack,&ImportMacroStackContextMethodTable)
				,createDefaultQueueNextExpression(libraryRestExpressionQueue)
				,nextCodegen->globalEnv
				,rest->curline
		...
		fklPushPtrStack(createCodegenQuest(_library_export_macro_with_prefix_pc_process
					,createCodegenQuestContext(exportMacroBcStack
				...
				if(curExp->type==FKL_NAST_SYM)
				{
					...
					else
					{
						...
						else
						{
							...
							if(fklIsSymbolDefined(curExp->u.sym,curEnv->cscope,curEnv))
							{
...
void fklCodegenPrintUndefinedSymbol(FklByteCodelnt* code,FklCodegenLib** libs,FklSymbolTable* symbolTable,size_t exportNum,FklSid_t* exports)
{
							...
							case FKL_OP_POP_VAR:
								{
									...
									else
									{
										...
										{
											r=fklIsSymbolDefined(id,e->cscope,e);
											if(r)
												break;
										}
										if(!r)
	...
	{
		if(!fklIsSymbolDefined(exports[i],mainEnv->cscope,mainEnv))
	...
	for(uint32_t i=0;i<mayUndefined->top;i++)
	{
		...
		for(FklCodegenEnv* e=curEnv;e;e=e->prev)
		{
			r=fklIsSymbolDefined(cur->sid,mainEnv->cscope,e);
			if(r)
				break;
		}
		if(!r)
