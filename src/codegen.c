...
static FklCodegenNextExpression *
createDefaultQueueNextExpression(FklPtrQueue *queue) {
...
static FklCodegenNextExpression *
createMustHasRetvalQueueNextExpression(FklPtrQueue *queue) {
    return createCodegenNextExpression(
        &_default_codegen_next_expression_method_table, queue,
        ALL_MUST_HAS_RETVAL);
}
...
static FklCodegenNextExpression *
createFirstHasRetvalQueueNextExpression(FklPtrQueue *queue) {
    return createCodegenNextExpression(
        &_default_codegen_next_expression_method_table, queue,
...
#define FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(F, STACK, NEXT_EXPRESSIONS,    \
    ...
    fklPushPtrStack(fklCreateCodegenQuest((F), (STACK), (NEXT_EXPRESSIONS),    \
                                          (SCOPE), (MACRO_SCOPE), (ENV),       \
                                          (LINE), NULL, (CODEGEN)),            \
                    (CODEGEN_CONTEXT))
...
static FklByteCodelnt *sequnce_exp_bc_process(FklPtrStack *stack, FklSid_t fid,
                                              uint32_t line, uint32_t scope) {
    if (stack->top) {
...
static inline void pushListItemToQueue(FklNastNode *list, FklPtrQueue *queue,
                                       FklNastNode **last) {
...
static inline void pushListItemToStack(FklNastNode *list, FklPtrStack *stack,
                                       FklNastNode **last) {
    for (; list->type == FKL_NAST_PAIR; list = list->pair->cdr)
...
static int pushFuncallListToQueue(FklNastNode *list, FklPtrQueue *queue,
                                  FklNastNode **last,
                                  FklNastNode *const *builtin_pattern_node) {
    for (; list->type == FKL_NAST_PAIR; list = list->pair->cdr)
        fklPushPtrQueue(fklMakeNastNodeRef(list->pair->car), queue);
...
static void codegen_funcall(FklNastNode *rest, FklPtrStack *codegenQuestStack,
...
#define CODEGEN_ARGS                                                           \
    FklNastNode *origExp, FklHashTable *ht, FklPtrStack *codegenQuestStack,    \
        uint32_t scope, FklCodegenMacroScope *macroScope,                      \
        FklCodegenEnv *curEnv, FklCodegenInfo *codegen,                        \
        FklCodegenOuterCtx *outer_ctx, FklCodegenErrorState *errorState,       \
...
static CODEGEN_FUNC(codegen_begin) {
    FklNastNode *rest =
        fklPatternMatchingHashTableRef(outer_ctx->builtInPatternVar_rest, ht);
    FklPtrQueue *queue = fklCreatePtrQueue();
...
static inline void process_unresolve_ref(FklCodegenEnv *env, uint32_t scope,
    ...
    fklInitPtrStack(&urefs1, count, 8);
...
static CODEGEN_FUNC(codegen_let1) {
    ...
    FklPatternMatchingHashTableItem *item =
        fklGetHashItem(&outer_ctx->builtInPatternVar_args, ht);
    ...
    if (args) {
        ...
        for (FklNastNode *cur = args; cur->type == FKL_NAST_PAIR;
             cur = cur->pair->cdr)
            fklPushPtrQueue(fklMakeNastNodeRef(cadr_nast_node(cur->pair->car)),
                            valueQueue);
    }
...
static CODEGEN_FUNC(codegen_let81) {
    FklSid_t letHeadId =
        fklAddSymbolCstr("let", &outer_ctx->public_symbol_table)->id;
    FklNastNode *letHead =
    ...
    FklNastNode *args =
        fklPatternMatchingHashTableRef(outer_ctx->builtInPatternVar_args, ht);
    FklNastNode *rest =
        fklPatternMatchingHashTableRef(outer_ctx->builtInPatternVar_rest, ht);
    ...
    FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(
        _default_bc_process, createDefaultStackContext(fklCreatePtrStack(1, 1)),
        createDefaultQueueNextExpression(queue), scope, macroScope, curEnv,
...
static CODEGEN_FUNC(codegen_letrec) {
    ...
    FklCodegenQuest *let1Quest = fklCreateCodegenQuest(
        ...
        createDefaultStackContext(fklCreatePtrStack(2, 16)), NULL, cs, cms,
        curEnv, origExp->curline, NULL, codegen);
...
static CODEGEN_FUNC(codegen_do0) {
    FklNastNode *cond =
        fklPatternMatchingHashTableRef(outer_ctx->builtInPatternVar_cond, ht);
    ...
    FklCodegenQuest *do0Quest = fklCreateCodegenQuest(
        _do0_exp_bc_process,
        createDefaultStackContext(fklCreatePtrStack(3, 16)), NULL, cs, cms,
    ...
    FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(
        ...
        createDefaultStackContext(
            fklCreatePtrStack(fklLengthPtrQueue(queue), 16)),
...
static CODEGEN_FUNC(codegen_do1) {
    ...
    FklCodegenQuest *do1Quest = fklCreateCodegenQuest(
        _do1_bc_process, createDefaultStackContext(fklCreatePtrStack(5, 16)),
        NULL, cs, cms, curEnv, origExp->curline, NULL, codegen);
    ...
    FklCodegenQuest *do1RestQuest =
        fklCreateCodegenQuest(_do_rest_exp_bc_process,
                              createDefaultStackContext(fklCreatePtrStack(
                                  fklLengthPtrQueue(queue), 16)),
                              createDefaultQueueNextExpression(queue), cs, cms,
...
static inline FklByteCodelnt *
process_set_var(FklPtrStack *stack, FklCodegenInfo *codegen,
...
BC_PROCESS(_def_var_exp_bc_process) {
    ...
    fklPushFrontPtrStack(append_put_loc_ins(INS_APPEND_BACK, NULL, idx,
                                            codegen->fid, ctx->line, scope),
                         stack);
...
static CODEGEN_FUNC(codegen_named_let0) {
    ...
    FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(
        _funcall_exp_bc_process,
        createDefaultStackContext(fklCreatePtrStack(2, 1)), NULL, cs, cms,
    ...
    fklPushPtrStack(append_put_loc_ins(INS_APPEND_BACK, NULL, idx, codegen->fid,
                                       name->curline, scope),
                    stack);
...
static CODEGEN_FUNC(codegen_named_let1) {
    ...
    fklPushPtrStack(
        fklCreateCodegenQuest(
            _let_arg_exp_bc_process,
            createDefaultStackContext(
                fklCreatePtrStack(fklLengthPtrQueue(valueQueue), 16)),
            createMustHasRetvalQueueNextExpression(valueQueue), scope,
            macroScope, curEnv, firstSymbol->curline, funcallQuest, codegen),
        codegenQuestStack);

    fklPushPtrStack(fklCreateCodegenQuest(_named_let_set_var_exp_bc_process,
                                          createDefaultStackContext(stack),
                                          NULL, cs, cms, curEnv, name->curline,
                                          funcallQuest, codegen),
                    codegenQuestStack);
...
FklCodegenEnv *fklCreateCodegenEnv(FklCodegenEnv *prev, uint32_t pscope,
    ...
    fklInitPtrStack(&r->ref_pdef, 8, 8);
...
void fklDestroyCodegenEnv(FklCodegenEnv *env) {
    while (env) {
        ...
        if (!env->refcount) {
            ...
            FklPtrStack *ref_pdef = &cur->ref_pdef;
            while (!fklIsPtrStackEmpty(ref_pdef))
                free(fklPopPtrStack(ref_pdef));
            fklUninitPtrStack(ref_pdef);
            free(cur);
        } else
            return;
    }
...
static CODEGEN_FUNC(codegen_setq) {
    ...
    if (def) {
        ...
        fklPushPtrStack(append_put_loc_ins(INS_APPEND_BACK, NULL, def->idx,
        ...
        fklPushPtrStack(append_put_var_ref_ins(INS_APPEND_BACK, NULL, def->idx,
                                               codegen->fid, name->curline,
                                               scope),
                        stack);
    }
    FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(
...
static inline void
push_default_codegen_quest(FklNastNode *value, FklPtrStack *codegenQuestStack,
                           uint32_t scope, FklCodegenMacroScope *macroScope,
                           FklCodegenEnv *curEnv, FklCodegenQuest *prev,
                           FklCodegenInfo *codegen) {
    FklPtrStack *stack = fklCreatePtrStack(1, 1);
    fklPushPtrStack(create_bc_lnt(fklCodegenNode(value, codegen), codegen->fid,
                                  value->curline, scope),
                    stack);
...
static inline void push_single_bcl_codegen_quest(
    FklByteCodelnt *bcl, FklPtrStack *codegenQuestStack, uint32_t scope,
                        ...
                        if (fklPatternMatch(
                            ...
                            if (rest->type == FKL_NAST_NIL)
                                ...
                                while (!fklIsPtrStackEmpty(&tmpStack))
                                    fklPushPtrStack(fklPopPtrStack(&tmpStack),
                                                    &expStack);
                                continue;
                            } else {
                                errorState->type = FKL_ERR_SYNTAXERROR;
...
static inline void
unquoteHelperFunc(FklNastNode *value, FklPtrStack *codegenQuestStack,
                  uint32_t scope, FklCodegenMacroScope *macroScope,
    ...
    fklPushPtrQueue(fklMakeNastNodeRef(value), queue);
    FklCodegenQuest *quest = fklCreateCodegenQuest(
        func, createDefaultStackContext(fklCreatePtrStack(1, 1)),
        createMustHasRetvalQueueNextExpression(queue), scope, macroScope,
...
static inline QsquoteHelperStruct *
create_qsquote_helper_struct(QsquoteHelperEnum e, FklNastNode *node,
                             FklCodegenQuest *prev) {
    QsquoteHelperStruct *r =
        (QsquoteHelperStruct *)malloc(sizeof(QsquoteHelperStruct));
    FKL_ASSERT(r);
    r->e = e;
    r->node = node;
    r->prev = prev;
...
static CODEGEN_FUNC(codegen_qsquote) {
    ...
    fklPushPtrStack(create_qsquote_helper_struct(QSQUOTE_NONE, value, NULL),
                    &valueStack);
    ...
    while (!fklIsPtrStackEmpty(&valueStack)) {
        ...
        case QSQUOTE_NONE: {
            ...
            if (fklPatternMatch(
                    ...
                    curValue, unquoteHt)) {
                FklNastNode *unquoteValue = fklPatternMatchingHashTableRef(
                    outer_ctx->builtInPatternVar_value, unquoteHt);
                ...
                FklCodegenQuest *curQuest = fklCreateCodegenQuest(
                    ...
                    createDefaultStackContext(fklCreatePtrStack(2, 1)), NULL,
                    scope, macroScope, curEnv, curValue->curline, prevQuest,
                    codegen);
                ...
                for (;;) {
                    ...
                    if (fklPatternMatch(
                        ...
                        if (node->pair->cdr->type != FKL_NAST_NIL) {
                            ...
                            fklPushPtrStack(create_qsquote_helper_struct(
                                                QSQUOTE_UNQTESP_CAR,
                ...
                FklCodegenQuest *curQuest = fklCreateCodegenQuest(
                    ...
                    createDefaultStackContext(fklCreatePtrStack(1, 1)), NULL,
                    scope, macroScope, curEnv, curValue->curline, prevQuest,
                    codegen);
                fklPushPtrStack(curQuest, codegenQuestStack);
                fklPushPtrStack(create_qsquote_helper_struct(
...
static CODEGEN_FUNC(codegen_cond) {
    ...
    FklCodegenQuest *quest = fklCreateCodegenQuest(
        ...
        createDefaultStackContext(fklCreatePtrStack(32, 16)), NULL, scope,
...
static CODEGEN_FUNC(codegen_if1) {
    ...
    fklPushPtrStack(fklCreateCodegenQuest(
                        ...
                        curScope, cms, curEnv, origExp->curline, prev, codegen),
                    codegenQuestStack);
}
...
static inline void
codegen_when_unless(FklHashTable *ht, uint32_t scope,
                    FklCodegenMacroScope *macroScope, FklCodegenEnv *curEnv,
                    FklCodegenInfo *codegen, FklPtrStack *codegenQuestStack,
                    FklByteCodeProcesser func, FklCodegenErrorState *errorState,
                    FklNastNode *origExp) {
    FklNastNode *cond = fklPatternMatchingHashTableRef(
        codegen->outer_ctx->builtInPatternVar_value, ht);

    FklNastNode *rest = fklPatternMatchingHashTableRef(
        codegen->outer_ctx->builtInPatternVar_rest, ht);
    FklPtrQueue *queue = fklCreatePtrQueue();
    ...
    FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(
        func, createDefaultStackContext(fklCreatePtrStack(32, 16)),
...
static void add_symbol_with_prefix_to_local_env_in_array(
    ...
    FklHashTable *exports, FklByteCodelnt *bcl, FklSid_t fid, uint32_t line,
    uint32_t scope, FklSymbolTable *pst) {
    FklStringBuffer buf;
...
static FklNastNode *createPatternWithPrefixFromOrig(FklNastNode *orig,
                                                    FklString *prefix,
                                                    FklSymbolTable *pst) {
    const FklString *head =
        fklGetSymbolWithId(orig->pair->car->sym, pst)->symbol;
    FklString *symbolWithPrefix = fklStringAppend(prefix, head);
    FklNastNode *patternWithPrefix =
        fklNastConsWithSym(fklAddSymbol(symbolWithPrefix, pst)->id,
...
void fklPrintUndefinedRef(const FklCodegenEnv *env,
    ...
    for (uint32_t i = urefs->top; i > 0; i--) {
        ...
        if (ref->fid) {
            fputs(" of ", stderr);
            fklPrintString(fklGetSymbolWithId(ref->fid, globalSymTable)->symbol,
                           stderr);
        }
        fputc('\n', stderr);
    }
...
static inline void recompute_ignores_terminal_sid_and_regex(
    ...
    FklRegexTable *target_rt, FklRegexTable *orig_rt) {
...
static inline void recompute_prod_terminal_sid(FklGrammerProduction *prod,
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (cur->isterm && cur->term_type != FKL_TERM_BUILTIN)
            cur->nt.sid =
                fklAddSymbol(
                    fklGetSymbolWithId(cur->nt.sid, origin_table)->symbol,
                    target_table)
                    ->id;
    }
}
...
import_reader_macro(int is_grammer_inited, int *p_is_grammer_inited,
                    ...
                    uint64_t curline, FklCodegenLib *lib,
                    FklGrammerProductionGroup *group, FklSid_t origin_group_id,
...
static inline FklByteCodelnt *process_import_imported_lib_common(
    uint32_t libId, FklCodegenInfo *codegen, FklCodegenLib *lib,
    ...
    if (lib->named_prod_groups.t) {
        ...
        for (FklHashTableItem *prod_group_item = lib->named_prod_groups.first;
             prod_group_item; prod_group_item = prod_group_item->next) {
            FklGrammerProductionGroup *group =
                (FklGrammerProductionGroup *)prod_group_item->data;
    ...
    if (lib->named_prod_groups.t) {
        ...
        for (FklHashTableItem *prod_group_item = lib->named_prod_groups.first;
            ...
            fklStringBufferConcatWithString(
                &buffer, fklGetSymbolWithId(group->id, pst)->symbol);

            FklSid_t group_id_with_prefix =
                fklAddSymbolCharBuf(buffer.buf, buffer.index, pst)->id;

            if (import_reader_macro(is_grammer_inited, &is_grammer_inited,
                                    codegen, errorState, curline, lib, group,
                                    group->id, group_id_with_prefix))
                break;
...
static inline FklByteCodelnt *process_import_imported_lib_only(
    ...
    for (; only->type == FKL_NAST_PAIR; only = only->pair->cdr) {
        ...
        if (lib->named_prod_groups.t
            && fklGetHashItem(&cur, &lib->named_prod_groups)) {
            r = 1;
            FklGrammerProductionGroup *group =
                fklGetHashItem(&cur, &lib->named_prod_groups);
            if (import_reader_macro(is_grammer_inited, &is_grammer_inited,
...
static inline void process_export_bc(FklCodegenInfo *info,
                                     ...
                                     uint32_t line, uint32_t scope) {
    append_export_to_ins(INS_APPEND_BACK, libBc, info->libStack->top + 1,
...
BC_PROCESS(_library_bc_process) {
    ...
    FklCodegenLib *lib = fklCreateCodegenScriptLib(codegen, libBc, env);
    ...
    return process_import_imported_lib(
        codegen->libStack->top, data->codegen, lib, data->env, data->scope,
...
BC_PROCESS(_export_import_bc_process) {
    ...
    FklByteCodelnt *bcl =
        export_sequnce_exp_bc_process(stack, fid, line, scope);
    if (bcl == NULL)
        return NULL;
...
static FklCodegenQuestContext *
create_export_sequnce_context(FklNastNode *origExp, FklPtrStack *s,
                              uint8_t must_has_retval) {
...
static CODEGEN_FUNC(codegen_export) {
    ...
    if (libCodegen && scope == 1 && curEnv->prev == codegen->global_env
        ...
        FKL_PUSH_NEW_DEFAULT_PREV_CODEGEN_QUEST(
            exports_bc_process,
            create_export_sequnce_context(orig_exp, fklCreatePtrStack(2, 1),
                                          must_has_retval),
...
static CODEGEN_FUNC(codegen_export_single) {
        ...
        fklPushPtrStack(
            fklCreateCodegenQuest(
                ...
                create_export_sequnce_context(
                    orig_exp, fklCreatePtrStack(2, 16), must_has_retval),
    ...
    process_def_in_lib:
        ...
        fklPushPtrStack(fklCreateCodegenQuest(
                            ...
                            createDefaultQueueNextExpression(queue), 1, cms,
                            curEnv, origExp->curline, NULL, codegen),
                        codegenQuestStack);
        ...
        fklPushPtrStack(fklCreateCodegenQuest(
                            ...
                            createExportContext(libCodegen, curEnv, 1, cms,
                                                NULL, NULL, NULL, NULL),
                            createDefaultQueueNextExpression(queue), 1, cms,
                            exportEnv, origExp->curline, NULL, codegen),
                        codegenQuestStack);
    } else {
    error:
        fklDestroyNastNode(value);
...
static inline void process_import_script_common_header(
    ...
    FklCodegenEnv *curEnv, FklCodegenInfo *codegen,
    FklCodegenErrorState *errorState, FklPtrStack *codegenQuestStack,
...
static inline FklByteCodelnt *process_import_from_dll_only(
    ...
    if (!libId) {
        ...
        lib = fklCreateCodegenDllLib(
            realpath, dll, codegen->runtime_symbol_table, initExport, pst);
        fklPushPtrStack(lib, codegen->libStack);
...
static inline void codegen_import_helper(
    ...
    FklCodegenInfo *codegen, FklCodegenErrorState *errorState,
    FklCodegenEnv *curEnv, uint32_t scope, FklCodegenMacroScope *macroScope,
    FklPtrStack *codegenQuestStack, FklNastNode *prefix, FklNastNode *only,
    FklNastNode *alias, FklNastNode *except, FklSymbolTable *pst) {
...
static CODEGEN_FUNC(codegen_import_none) {
    ...
    fklPushPtrStack(fklCreateCodegenQuest(
                        _empty_bc_process, createEmptyContext(), NULL, 1,
                        macroScope, curEnv, origExp->curline, NULL, codegen),
                    codegenQuestStack);
}
...
static void *custom_action(void *c, void *outerCtx, void *nodes[], size_t num,
    ...
    fklPatternMatchingHashTableSet(fklAddSymbolCstr("$$", ctx->pst)->id,
                                   nodes_vector, &ht);
    fklPatternMatchingHashTableSet(fklAddSymbolCstr("line", ctx->pst)->id,
                                   line_node, &ht);
...
static struct CstrIdProdAction {
    ...
    {
        "nil",
        codegen_prod_action_nil,
    },
    {
        "symbol",
        codegen_prod_action_symbol,
    },
    {
        "first",
        codegen_prod_action_first,
    },
    {
        "second",
        codegen_prod_action_second,
    },
    {
        "third",
        codegen_prod_action_third,
    },
    {
        "pair",
        codegen_prod_action_pair,
    },
    {
        "cons",
        codegen_prod_action_cons,
    },
    {
        "box",
        codegen_prod_action_box,
    },
    {
        "vector",
        codegen_prod_action_vector,
    },
    {
        "quote",
        codegen_prod_action_quote,
    },
    {
        "unquote",
        codegen_prod_action_unquote,
    },
    {
        "qsquote",
        codegen_prod_action_qsquote,
    },
    {
        "unqtesp",
        codegen_prod_action_unqtesp,
    },
    {
        "hasheq",
        codegen_prod_action_hasheq,
    },
    {
        "hasheqv",
        codegen_prod_action_hasheqv,
    },
    {
        "hashequal",
        codegen_prod_action_hashequal,
    },
    {
        "bytevector",
        codegen_prod_action_bytevector,
    },
};
...
init_builtin_prod_action_list(FklSid_t *builtin_prod_action_id,
    ...
    for (size_t i = 0; i < FKL_CODEGEN_BUILTIN_PROD_ACTION_NUM; i++)
        builtin_prod_action_id[i] =
            fklAddSymbolCstr(CodegenProdActions[i].name, pst)->id;
}
...
static void *simple_action_symbol(void *c, void *outerCtx, void *nodes[],
    ...
    if (ctx->start) {
        ...
        FklSid_t id =
            fklAddSymbolCharBuf(buffer.buf, buffer.index, outerCtx)->id;
...
static inline void init_simple_prod_action_list(FklSid_t *simple_prod_action_id,
    ...
    for (size_t i = 0; i < FKL_CODEGEN_SIMPLE_PROD_ACTION_NUM; i++)
        simple_prod_action_id[i] =
            fklAddSymbolCstr(CodegenProdCreatorActions[i].name, pst)->id;
}
...
static inline FklGrammerSym *nast_vector_to_production_right_part(
    const FklNastVector *vec, FklHashTable *builtin_term, FklSymbolTable *tt,
    FklRegexTable *rt, size_t *num, int *failed) {
    ...
    for (size_t i = 0; i < vec->size; i++) {
        ...
        if (cur->type == FKL_NAST_STR)
            ...
            fklPushPtrStack((void *)cur, &valid_items);
        else if (cur->type == FKL_NAST_VECTOR && cur->vec->size == 1
...
nast_vector_to_ignore(const FklNastVector *vec, FklHashTable *builtin_term,
                      FklSymbolTable *tt, FklRegexTable *rt, int *failed) {
    size_t num = 0;
...
macro_compile_prepare(FklCodegenInfo *codegen, FklCodegenMacroScope *macroScope,
                      FklHashTable *symbolSet, FklCodegenEnv **pmacroEnv,
    ...
    macroCodegen->fid = macroCodegen->filename
                          ? fklAddSymbolCstr(macroCodegen->filename, pst)->id
                          : 0;
    ...
    for (FklHashTableItem *list = symbolSet->first; list; list = list->next) {
...
static inline FklCodegenProdPrinting *
create_codegen_prod_printing(FklSid_t group, FklSid_t sid, FklNastNode *vec,
                             uint8_t type, uint8_t add_extra,
                             FklNastNode *forth, uint32_t prototype_id,
                             FklByteCodelnt *bcl) {
    FklCodegenProdPrinting *p =
        (FklCodegenProdPrinting *)malloc(sizeof(FklCodegenProdPrinting));
    FKL_ASSERT(p);
    p->group_id = group;
    p->sid = sid;
...
BC_PROCESS(process_adding_production) {
    ...
    if (group_id == 0) {
        ...
        FklCodegenProdPrinting *p = create_codegen_prod_printing(
    ...
    if (codegen->export_named_prod_groups
        ...
        && ((FklGrammerProductionGroup *)fklGetHashItem(
                &group_id, codegen->named_prod_groups))
               ->is_ref_outer) {
        errorState->type = FKL_ERR_EXPORT_OUTER_REF_PROD_GROUP;
        errorState->line = line;
        errorState->place = NULL;
...
static inline FklGrammerProduction *nast_vector_to_production(
    ...
    FklHashTable *builtin_term, FklSymbolTable *tt, FklRegexTable *rt,
    FklSid_t left_group, FklSid_t sid, FklSid_t action_type,
    FklNastNode *action_ast, FklSid_t group_id, FklCodegenInfo *codegen,
    FklCodegenEnv *curEnv, FklCodegenMacroScope *macroScope,
    FklPtrStack *codegenQuestStack, FklSymbolTable *pst, int *failed) {
    size_t len = 0;
...
static inline int add_all_ignores_to_grammer(FklCodegenInfo *codegen) {
    ...
    for (FklHashTableItem *group_items = codegen->named_prod_groups->first;
        ...
        FklGrammerProductionGroup *group =
            (FklGrammerProductionGroup *)group_items->data;
            ...
            for (size_t i = 0; i < len; i++) {
                ...
                if (!cur->isterm
                    && !fklIsNonterminalExist(&g->productions, cur->nt.group,
                                              cur->nt.sid)) {
                    if (cur->nt.group) {
                        FklGrammerProductionGroup *group = fklGetHashItem(
                            &cur->nt.group, codegen->named_prod_groups);
                        if (!group)
                            return 1;
...
static inline FklGrammerProductionGroup *
get_production_group(const FklHashTable *ht, FklSid_t group) {
...
static inline int
process_add_production(FklSid_t group_id, FklCodegenInfo *codegen,
                       FklNastNode *vector_node,
                       FklCodegenErrorState *errorState, FklCodegenEnv *curEnv,
                       FklCodegenMacroScope *macroScope,
                       FklPtrStack *codegenQuestStack, FklSymbolTable *pst) {
    FklGrammer *g = *codegen->g;
    ...
    if (vec->size == 1) {
        ...
        if (group_id == 0) {
            ...
            fklPushPtrStack(fklMakeNastNodeRef(vec->base[0]),
                            &group->ignore_printing);
        }
    } else if (vec->size == 4) {
...
static CODEGEN_FUNC(codegen_def_reader_macros) {
    ...
    for (FklQueueNode *first = prod_vector_queue.head; first;
         first = first->next)
    ...
    while (!fklIsPtrStackEmpty(&stack)) {
        ...
        case FKL_NAST_SYM:
            append_push_sym_ins_to_bc(
                ...
                fklAddSymbol(fklGetSymbolWithId(node->sym, pst)->symbol,
...
static inline int matchAndCall(FklCodegenFunc func, const FklNastNode *pattern,
                               uint32_t scope, FklCodegenMacroScope *macroScope,
                               FklNastNode *exp, FklPtrStack *codegenQuestStack,
...
static struct PatternAndFunc {
    ...
    {
        "~(do [(~name ~arg0),~args] [~cond ~value],~rest)",
        codegen_do1,
    },
    {
        "~(do [(~name ~arg0),~args] [~cond],~rest)",
        codegen_do1,
    },

    {
        "~(do [(~name ~arg0 ~arg1),~args] [~cond ~value],~rest)",
        codegen_do1,
    },
    {
        "~(do [(~name ~arg0 ~arg1),~args] [~cond],~rest)",
        codegen_do1,
    },
    ...
    {
        "~(import (prefix ~name ~rest),~args)",
        codegen_import_prefix,
    },
    {
        "~(import (only ~name,~rest),~args)",
        codegen_import_only,
    },
    {
        "~(import (alias ~name,~rest),~args)",
        codegen_import_alias,
    },
    {
        "~(import (except ~name,~rest),~args)",
        codegen_import_except,
    },
    {
        "~(import (common ~name),~args)",
        codegen_import_common,
    },
    {
        "~(import ~name,~args)",
        codegen_import,
    ...
    {
        "~(defmacro ~arg0 ~arg1,~rest)",
        codegen_def_reader_macros,
    },
    {
        "~(cfg ~cond ~value,~rest)",
        codegen_cond_compile,
    },
    {
        NULL,
        NULL,
...
void fklInitCodegenOuterCtx(FklCodegenOuterCtx *outerCtx,
    ...
    outerCtx->builtInPatternVar_orig =
        fklAddSymbolCstr("orig", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_rest =
        fklAddSymbolCstr("rest", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_name =
        fklAddSymbolCstr("name", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_cond =
        fklAddSymbolCstr("cond", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_args =
        fklAddSymbolCstr("args", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_arg0 =
        fklAddSymbolCstr("arg0", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_arg1 =
        fklAddSymbolCstr("arg1", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_value =
        fklAddSymbolCstr("value", publicSymbolTable)->id;

    outerCtx->builtInPatternVar_custom =
        fklAddSymbolCstr("custom", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_builtin =
        fklAddSymbolCstr("builtin", publicSymbolTable)->id;
    outerCtx->builtInPatternVar_simple =
        fklAddSymbolCstr("simple", publicSymbolTable)->id;
...
static inline int
mapAllBuiltInPattern(FklNastNode *curExp, FklPtrStack *codegenQuestStack,
                     uint32_t scope, FklCodegenMacroScope *macroScope,
                     FklCodegenEnv *curEnv, FklCodegenInfo *codegenr,
                     FklCodegenErrorState *errorState,
                     uint8_t must_has_retval) {
    FklNastNode *const *builtin_pattern_node =
    ...
    while (!fklIsPtrStackEmpty(&codegenQuestStack)) {
        ...
        if (nextExpression) {
            ...
            skip:
                ...
                fklDestroyNastNode(curExp);
                if ((!r
                     && fklTopPtrStack(&codegenQuestStack) != curCodegenQuest)
                    || errorState.type)
                    break;
            }
        }
        ...
        if (otherCodegenQuest == curCodegenQuest) {
            fklPopPtrStack(&codegenQuestStack);
            FklCodegenQuest *prevQuest = curCodegenQuest->prev
                                           ? curCodegenQuest->prev
                                           : fklTopPtrStack(&codegenQuestStack);
...
fklInitGlobalCodegenInfo(FklCodegenInfo *codegen, const char *rp,
    ...
    if (rp != NULL) {
        ...
        codegen->fid = fklAddSymbolCstr(codegen->filename, runtime_st)->id;
    } else {
...
FklCodegenLib *fklCreateCodegenScriptLib(FklCodegenInfo *codegen,
    ...
    FKL_ASSERT(lib);
    fklInitCodegenScriptLib(lib, codegen, bcl, env);
    return lib;
}
...
static FklCodegenMacro *
findMacro(FklNastNode *exp, FklCodegenMacroScope *macros, FklHashTable **pht) {
    if (!exp)
        return NULL;
...
static void initVMframeFromPatternMatchTable(FklVM *exe, FklVMframe *frame,
    ...
    for (FklHashTableItem *list = ht->first; list; list = list->next) {
        FklPatternMatchingHashTableItem *item =
            (FklPatternMatchingHashTableItem *)list->data;
...
FklVM *fklInitMacroExpandVM(FklByteCodelnt *bcl, FklFuncPrototypes *pts,
    ...
    anotherVM->libs =
        (FklVMlib *)calloc(macroLibStack->top + 1, sizeof(FklVMlib));
...
FklNastNode *fklTryExpandCodegenMacro(FklNastNode *exp, FklCodegenInfo *codegen,
    ...
    for (FklCodegenMacro *macro = findMacro(r, macros, &ht);
        ...
        fklPatternMatchingHashTableSet(
            codegen->outer_ctx->builtInPatternVar_orig, exp, ht);
...
void fklInitVMlibWithCodegenLib(FklCodegenLib *clib, FklVMlib *vlib, FklVM *exe,
                                int needCopy, FklFuncPrototypes *pts) {
    FklVMvalue *val = FKL_VM_NIL;
    if (clib->type == FKL_CODEGEN_LIB_SCRIPT) {
        FklByteCode *bc = clib->bcl->bc;
        FklVMvalue *codeObj = fklCreateVMvalueCodeObj(
...
static inline void write_ignores(FklPtrStack *igs, const FklSymbolTable *st,
                                 FILE *fp) {
...
static inline void write_prods(FklPtrStack *prods, const FklSymbolTable *st,
                               FILE *fp) {
...
void fklWriteExportNamedProds(const FklHashTable *export_named_prod_groups,
    ...
    if (!has_named_prod)
        return;
    fwrite(&export_named_prod_groups->num,
           sizeof(export_named_prod_groups->num), 1, fp);
    for (FklHashTableItem *list = export_named_prod_groups->first; list;
         list = list->next) {
        FklSid_t id = *(FklSid_t *)list->data;
        FklGrammerProductionGroup *group =
