...
FklByteCodelnt* fklCompileGetf(FklAstCptr* objCptr,FklCompEnv* curEnv,FklIntpr* inter,FklErrorState* state,int evalIm)
{
	...
	if(GET_TYPES_TAG(typeUnion.all)!=FUNC_TYPE_TAG)
	{
		...
		{
			if(pathCptr->type!=ATM||(pathCptr->u.atom->type!=SYM&&pathCptr->u.atom->type!=FKL_I32&&pathCptr->u.atom->type!=FKL_I64))
			{
				state->state=SYNTAXERROR;
...
//					FklAstCptr* tmpCptr=fklNextCptr(fir);
//					FklAstAtom* tmpAtm=tmpCptr->u.atom;
//					if(tmpAtm->type!=SYM&&tmpAtm->type!=FKL_STR)
//					{
//						state->place=tmpCptr;
//						state->state=SYNTAXERROR;
//						uint32_t i=0;
...
//						FklCompEnv* tmpEnv=curEnv;
//						FklCompDef* tmpDef=NULL;
//						if(tmpAtm->type!=FKL_STR&&tmpAtm->type!=SYM)
//						{
//							state->place=tmpCptr;
//							state->state=SYMUNDEFINE;
//							uint32_t i=0;
	...
	for(;plib;plib=fklNextCptr(plib))
	{
		...
		while(pPartsOfPath)
		{
			...
			if(!strcmp(tmpAtm->value.str,"prefix")&&fklNextCptr(pPartsOfPath)->type==PAIR)
			{
				...
				if(prefixAtom->type!=FKL_STR&&prefixAtom->type!=SYM)
				{
					state->state=SYNTAXERROR;
					state->place=plib;
					FREE_ALL_LINE_NUMBER_TABLE(tmp->l,tmp->ls);
