...
{
	FKL_DECL_UD_DATA(da,Dvec,a);
	FKL_DECL_UD_DATA(db,Dvec,b);
	if(da->size!=db->size)
		return 0;
...
static inline void dvec_reserve(Dvec* v,size_t target)
{
...
static int _dvec_append(FklVMud* ud,uint32_t argc,FklVMvalue* const* top)
{
	...
	for(int64_t i=0;i<argc;i++)
	{
		...
		if(is_dvec_ud(cur))
		{
			FKL_DECL_VM_UD_DATA(d,Dvec,cur);
			new_size+=d->size;
		}
...
static inline FklVMvalue* create_dvec(FklVM* exe,size_t size,FklVMvalue* dll)
{
	FklVMvalue* r=fklCreateVMvalueUd(exe,&DvecMetaTable,dll);
	FKL_DECL_VM_UD_DATA(v,Dvec,r);
	v->size=size;
	v->capacity=size;
	if(size==0)
		v->base=NULL;
...
static inline FklVMvalue* create_dvec_with_capacity(FklVM* exe,size_t capacity,FklVMvalue* dll)
{
	FklVMvalue* r=fklCreateVMvalueUd(exe,&DvecMetaTable,dll);
	FKL_DECL_VM_UD_DATA(v,Dvec,r);
	v->size=0;
...
static inline FklVMvalue* create_dvec2(FklVM* exe,size_t size,FklVMvalue* const* ptr,FklVMvalue* dll)
{
	FklVMvalue* r=fklCreateVMvalueUd(exe,&DvecMetaTable,dll);
	FKL_DECL_VM_UD_DATA(v,Dvec,r);
	v->size=size;
...
static FklVMvalue* _dvec_copy_append(FklVM* exe,const FklVMud* v,uint32_t argc,FklVMvalue* const* top)
{
	FKL_DECL_UD_DATA(dvec,Dvec,v);
	size_t new_size=dvec->size;
	for(int64_t i=0;i<argc;i++)
	{
		FklVMvalue* cur=top[-i];
	...
	FKL_DECL_VM_UD_DATA(new_vec,Dvec,new_vec_val);
	new_size=dvec->size;
	if(new_vec->base)
...
static int export_make_dvec(FKL_CPROC_ARGL)
{
	...
	if(!content)
		content=FKL_VM_NIL;
	FKL_DECL_VM_UD_DATA(vec,Dvec,r);
...
static int export_make_dvec_with_capacity(FKL_CPROC_ARGL)
{
	...
	if(fklIsVMnumberLt0(size))
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_NUMBER_SHOULD_NOT_BE_LT_0,exe);
	size_t len=fklVMgetUint(size);
	FklVMvalue* r=create_dvec_with_capacity(exe,len,FKL_VM_CPROC(ctx->proc)->dll);
	FKL_VM_PUSH_VALUE(exe,r);
	return 0;
}
...
static int export_dvec_empty_p(FKL_CPROC_ARGL)
{
	...
	FKL_DECL_VM_UD_DATA(v,Dvec,obj);
...
static int export_subdvec(FKL_CPROC_ARGL)
{
	...
	if(fklIsVMnumberLt0(vstart)||fklIsVMnumberLt0(vend))
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_NUMBER_SHOULD_NOT_BE_LT_0,exe);
	FKL_DECL_VM_UD_DATA(vec,Dvec,ovec);
	...
	if(start>size||end<start||end>size)
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INVALIDACCESS,exe);
	size=end-start;
	FklVMvalue* r=create_dvec2(exe,size,vec->base+start,FKL_VM_CPROC(ctx->proc)->dll);
	FKL_VM_PUSH_VALUE(exe,r);
	return 0;
}
...
static int export_sub_dvec(FKL_CPROC_ARGL)
{
	...
	if(start+osize>size)
		FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INVALIDACCESS,exe);
	FklVMvalue* r=create_dvec2(exe,osize,vec->base+start,FKL_VM_CPROC(ctx->proc)->dll);
	FKL_VM_PUSH_VALUE(exe,r);
	return 0;
}
		...
		case 1:
			{
				...
				{
					FKL_DECL_VM_UD_DATA(v,Dvec,vec);
					FKL_DECL_VM_UD_DATA(another_vec,Dvec,another_vec_val);
					dvec_reserve(v,another_vec->size);
					memcpy(v->base,another_vec->base,another_vec->size*sizeof(FklVMvalue*));
					v->size=another_vec->size;
				}
		...
		case 4:
			{
				...
				{
					FKL_DECL_VM_UD_DATA(v,Dvec,another_vec_val);
					if(start_idx+count>v->size)
						FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INVALIDACCESS,exe);
					src_mem=&v->base[start_idx];
				}
...
static int export_vector_to_dvec(FKL_CPROC_ARGL)
{
	...
	FKL_VM_PUSH_VALUE(exe,create_dvec2(exe,vec->size,vec->base,FKL_VM_CPROC(ctx->proc)->dll));
	return 0;
}
