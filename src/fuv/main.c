...
static inline void cleanup_handle(FklVMvalue *handle_obj,
    ...
    free(fuv_handle);
    if (handle_obj) {
        fklVMvalueUsetDel2(&ldata->gc_values, handle_obj);
        *handle_ud = NULL;
    }
}
...
static int fuv_handle_active_p(FKL_CPROC_ARGL) {
    ...
    FKL_VM_PUSH_VALUE(exe, uv_is_active(GET_HANDLE(*fuv_handle)) ? FKL_VM_TRUE
                                                                 : FKL_VM_NIL);
    return 0;
}
...
static int fuv_handle_closing_p(FKL_CPROC_ARGL) {
    ...
    FKL_CHECK_TYPE(handle_obj, isFuvHandle, exe);
    DECL_FUV_HANDLE_UD_AND_CHECK_CLOSED(fuv_handle, handle_obj, exe, ctx->pd);
    FKL_VM_PUSH_VALUE(exe, uv_is_closing(GET_HANDLE(*fuv_handle)) ? FKL_VM_TRUE
                                                                  : FKL_VM_NIL);
    return 0;
}
...
static int fuv_handle_ref_p(FKL_CPROC_ARGL) {
    ...
    DECL_FUV_HANDLE_UD_AND_CHECK_CLOSED(fuv_handle, handle_obj, exe, ctx->pd);
    FuvHandle *handle = *fuv_handle;
    FKL_VM_PUSH_VALUE(exe, uv_has_ref(GET_HANDLE(handle)) ? FKL_VM_TRUE
                                                          : FKL_VM_NIL);
    return 0;
}
...
static int fuv_handle_type(FKL_CPROC_ARGL) {
    ...
    FKL_VM_PUSH_VALUE(
        exe, name == NULL ? FKL_VM_NIL
                          : fklCreateVMvaluePair(
                                exe, fklCreateVMvalueStrFromCstr(exe, name),
...
static int fuv_timer_repeat(FKL_CPROC_ARGL) {
    ...
    FKL_VM_PUSH_VALUE(
        exe, fklMakeVMuint(
                 uv_timer_get_repeat((uv_timer_t *)GET_HANDLE(handle)), exe));
    return 0;
}
...
static int fuv_timer_due_in(FKL_CPROC_ARGL) {
    ...
    FKL_VM_PUSH_VALUE(
        exe, fklMakeVMuint(
                 uv_timer_get_due_in((uv_timer_t *)GET_HANDLE(handle)), exe));
    return 0;
}
...
pop_addrinfo_hints(FklVM *exe, struct addrinfo *hints, FuvPublicData *fpd) {
    FklVMvalue *cur = FKL_VM_POP_ARG(exe);
    while (cur) {
        if (FKL_IS_SYM(cur)) {
...
static int fuv_getaddrinfo(FKL_CPROC_ARGL) {
    ...
    if (!proc_obj || proc_obj == FKL_VM_NIL) {
        ...
        FKL_VM_SET_TP_AND_PUSH_VALUE(exe, rtp,
                                     addrinfo_to_value(exe, req.addrinfo, fpd));
...
static inline FklBuiltinErrorType
pop_sockaddr_flags(FklVM *exe, struct sockaddr_storage *addr, int *flags,
...
static inline FklVMvalue *host_service_to_hash(FklVM *exe, const char *hostname,
    ...
    fklVMhashTableSet(
        ...
        hostname ? fklCreateVMvalueStrFromCstr(exe, hostname) : FKL_VM_NIL, ht);
    fklVMhashTableSet(
        FKL_MAKE_VM_SYM(fpd->f_service_sid),
        service ? fklCreateVMvalueStrFromCstr(exe, service) : FKL_VM_NIL, ht);
    return hash;
}
...
static int fuv_getnameinfo(FKL_CPROC_ARGL) {
    ...
    FklBuiltinErrorType error_type =
        pop_sockaddr_flags(exe, &addr, &flags, fpd, &ip_obj, &port_obj, &r);
    if (error_type)
    ...
    if (!proc_obj || proc_obj == FKL_VM_NIL) {
        ...
        FKL_VM_SET_TP_AND_PUSH_VALUE(
            exe, rtp, host_service_to_hash(exe, req.host, req.service, fpd));
    } else {
        FklVMvalue *retval = NULL;
        uv_getnameinfo_t *req =
...
static inline FklBuiltinErrorType
pop_process_options(FklVM *exe, uv_process_options_t *options,
                    FuvPublicData *fpd, int *uv_err, FklVMvalue **args_obj,
                    FklVMvalue **env_obj, FklVMvalue **stdio_obj,
                    FklVMvalue **cwd_obj, FuvErrorType *fuv_err) {
    FklVMvalue *cur = FKL_VM_POP_ARG(exe);
    while (cur) {
        if (FKL_IS_SYM(cur)) {
            FklSid_t id = FKL_GET_SYM(cur);
...
static int fuv_process_spawn(FKL_CPROC_ARGL) {
    ...
    FklBuiltinErrorType fkl_err =
        pop_process_options(exe, &options, fpd, &uv_err, &env_obj, &args_obj,
                            &stdio_obj, &cwd_obj, &fuv_err);
...
static inline FklBuiltinErrorType setup_write_data(FklVM *exe, uint32_t num,
...
static int fuv_stream_write(FKL_CPROC_ARGL) {
    ...
    if (arg_num) {
        ...
        FklBuiltinErrorType err_type =
            setup_write_data(exe, arg_num, write_obj, &bufs);
        if (err_type)
            FKL_RAISE_BUILTIN_ERROR(err_type, exe);
    } else
    ...
    if (send_handle_obj) {
        ...
        ret =
            uv_write2(write, stream, bufs, arg_num,
...
static inline FklBuiltinErrorType setup_try_write_data(FklVM *exe, uint32_t num,
                                                       uv_buf_t **pbufs) {
    uv_buf_t *bufs = (uv_buf_t *)malloc(num * sizeof(uv_buf_t));
    FKL_ASSERT(bufs);
    FklVMvalue *val = FKL_VM_POP_ARG(exe);
...
static int fuv_stream_try_write(FKL_CPROC_ARGL) {
    ...
    if (arg_num) {
        ...
        if (isFuvStream(top_value)) {
            ...
            FKL_VM_PUSH_VALUE(exe, top_value);
        FklBuiltinErrorType err_type =
            setup_try_write_data(exe, arg_num, &bufs);
        if (err_type)
            FKL_RAISE_BUILTIN_ERROR(err_type, exe);
    } else
        FKL_RAISE_BUILTIN_ERROR(FKL_ERR_TOOFEWARG, exe);
    ...
    if (send_handle_obj) {
        ...
        ret = uv_try_write2(stream, bufs, arg_num,
...
static int fuv_pipe(FKL_CPROC_ARGL) {
    ...
    CHECK_UV_RESULT(ret, exe, ctx->pd);
    FKL_VM_PUSH_VALUE(exe, fklCreateVMvaluePair(exe, FKL_MAKE_VM_FIX(fds[0]),
...
static inline FklBuiltinErrorType pop_tcp_flags(FklVM *exe, FuvPublicData *fpd,
                                                unsigned int *flags) {
    FklVMvalue *cur = FKL_VM_POP_ARG(exe);
    while (cur) {
        if (FKL_IS_SYM(cur)) {
...
parse_sockaddr_with_fpd(FklVM *exe, struct sockaddr_storage *address,
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->f_family_sid),
                      af_num_to_symbol(address->ss_family, fpd), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->f_ip_sid),
                      fklCreateVMvalueStrFromCstr(exe, ip), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->f_port_sid), FKL_MAKE_VM_FIX(port),
                      ht);
...
static int fuv_socketpair(FKL_CPROC_ARGL) {
    FKL_DECL_AND_CHECK_ARG4(socktype_obj, protocol_obj, non_block_flags0_obj,
                            non_block_flags1_obj, exe);
    FKL_CHECK_REST_ARG(exe);
...
static int fuv_tty_winsize(FKL_CPROC_ARGL) {
    ...
    CHECK_UV_RESULT(r, exe, ctx->pd);
    FKL_VM_PUSH_VALUE(exe, fklCreateVMvaluePair(exe, FKL_MAKE_VM_FIX(width),
...
static inline FklBuiltinErrorType pop_udp_flags(FklVM *exe, unsigned int *flags,
                                                FuvPublicData *fpd) {
    FklVMvalue *cur = FKL_VM_POP_ARG(exe);
    while (cur) {
        if (FKL_IS_SYM(cur)) {
...
static int fuv_udp_membership_set1(FKL_CPROC_ARGL) {
    FKL_DECL_AND_CHECK_ARG4(udp_obj, multicast_addr_obj, interface_addr_obj,
                            membership_obj, exe);
    FKL_CHECK_REST_ARG(exe);
    FKL_CHECK_TYPE(udp_obj, isFuvUdp, exe);
...
static inline FklBuiltinErrorType setup_udp_send_data(FklVM *exe, uint32_t num,
                                                      FklVMvalue *write_obj,
                                                      uv_buf_t **pbufs) {
    FKL_DECL_VM_UD_DATA(fuv_req, FuvReqUd, write_obj);
...
static int fuv_udp_send(FKL_CPROC_ARGL) {
    ...
    if (arg_num) {
        udp_send = createFuvUdpSend(exe, &udp_send_obj, ctx->proc,
                                    handle->data.loop, cb_obj, arg_num);
    ...
    int ret = uv_udp_send(udp_send, (uv_udp_t *)GET_HANDLE(handle), bufs,
                          arg_num, addr_ptr, fuv_udp_send_cb);
    free(bufs);
...
static int fuv_udp_try_send(FKL_CPROC_ARGL) {
    ...
    int ret = uv_udp_try_send((uv_udp_t *)GET_HANDLE(*udp_ud), bufs, arg_num,
                              addr_ptr);
    free(bufs);
    CHECK_UV_RESULT(ret, exe, ctx->pd);
    FKL_VM_PUSH_VALUE(exe, FKL_MAKE_VM_FIX(ret));
...
static inline FklBuiltinErrorType
pop_fs_event_flags(FklVM *exe, unsigned int *flags, FklVMvalue *pd) {
    FKL_DECL_VM_UD_DATA(fpd, FuvPublicData, pd);
    FklVMvalue *cur = FKL_VM_POP_ARG(exe);
    while (cur) {
        if (FKL_IS_SYM(cur)) {
...
timespec_to_vmtable(FklVM *exe, const uv_timespec_t *spec, FuvPublicData *fpd) {
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->timespec_f_sec_sid),
                      fklMakeVMint(spec->tv_sec, exe), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->timespec_f_nsec_sid),
                      fklMakeVMint(spec->tv_nsec, exe), ht);
...
static inline FklVMvalue *dirent_to_vmtable(FklVM *exe, uv_dirent_t *d,
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->dirent_f_type_sid),
                      FKL_MAKE_VM_SYM(dirent_type_to_sid(d->type, fpd)), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->dirent_f_name_sid),
                      fklCreateVMvalueStrFromCstr(exe, d->name), ht);
...
static inline FklBuiltinErrorType setup_fs_write_buf(FklVM *exe,
                                                     FklStringBuffer *buf) {
    FklVMvalue *cur = FKL_VM_POP_ARG(exe);
    while (cur) {
        if (FKL_IS_STR(cur))
...
static int fuv_scandir_cb_value_creator(FklVM *exe, void *a) {
    ...
    if (fs->result < 0) {
        ...
        fklVMvalueUsetDel2(&fuv_loop->data.gc_values, req->data.req);
        req->data.loop = NULL;
        req->data.callback = NULL;
...
timeval_to_vmtable(FklVM *exe, const uv_timeval_t *spec, FuvPublicData *fpd) {
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->timeval_f_usec_sid),
                      fklMakeVMint(spec->tv_usec, exe), ht);
...
static inline FklVMvalue *cpu_info_to_vmtable(FklVM *exe, uv_cpu_info_t *info,
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_model_sid),
                      fklCreateVMvalueStrFromCstr(exe, info->model), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_speed_sid),
                      FKL_MAKE_VM_FIX(info->speed), ht);
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_times_user_sid),
                      fklMakeVMuint(info->cpu_times.user, exe), cht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_times_nice_sid),
                      fklMakeVMuint(info->cpu_times.nice, exe), cht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_times_sys_sid),
                      fklMakeVMuint(info->cpu_times.sys, exe), cht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_times_idle_sid),
                      fklMakeVMuint(info->cpu_times.idle, exe), cht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_times_irq_sid),
                      fklMakeVMuint(info->cpu_times.irq, exe), cht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->cpu_info_times_sid), cpu_times, ht);
    return hash;
}
...
interface_addresses_to_vec(FklVM *exe, uv_interface_address_t *addresses,
    ...
    for (int i = 0; i < count; i++) {
        ...
        fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->ifa_f_name_sid),
                          fklCreateVMvalueStrFromCstr(exe, cur->name), ht);

        fklVMhashTableSet(
            FKL_MAKE_VM_SYM(fpd->ifa_f_mac_sid),
            fklCreateVMvalueBvec2(exe, sizeof(cur->phys_addr),
                                  (const uint8_t *)cur->phys_addr),
            ht);

        fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->ifa_f_internal_sid),
                          cur->is_internal ? FKL_VM_TRUE : FKL_VM_NIL, ht);
        ...
        fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->ifa_f_netmask_sid),
                          fklCreateVMvalueStrFromCstr(exe, netmask), ht);
        fklVMhashTableSet(
            FKL_MAKE_VM_SYM(fpd->ifa_f_family_sid),
            af_num_to_symbol(cur->address.address4.sin_family, fpd), ht);
    }
    return v;
}
...
static int fuv_interface_address(FKL_CPROC_ARGL) {
    ...
    CHECK_UV_RESULT(r, exe, ctx->pd);
    FKL_VM_PUSH_VALUE(
        exe, interface_addresses_to_vec(exe, addresses, count, ctx->pd));
...
static inline FklVMvalue *passwd_to_vmtable(FklVM *exe, uv_passwd_t *passwd,
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->passwd_username_sid),
                      fklCreateVMvalueStrFromCstr(exe, passwd->username), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->passwd_uid_sid),
                      fklMakeVMint(passwd->uid, exe), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->passwd_gid_sid),
                      fklMakeVMint(passwd->gid, exe), ht);
...
static inline FklVMvalue *utsname_to_vmtable(FklVM *exe, uv_utsname_t *buf,
    ...
    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->utsname_sysname_sid),
                      fklCreateVMvalueStrFromCstr(exe, buf->sysname), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->utsname_release_sid),
                      fklCreateVMvalueStrFromCstr(exe, buf->release), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->utsname_version_sid),
                      fklCreateVMvalueStrFromCstr(exe, buf->version), ht);

    fklVMhashTableSet(FKL_MAKE_VM_SYM(fpd->utsname_machine_sid),
                      fklCreateVMvalueStrFromCstr(exe, buf->machine), ht);
    return hash;
}
