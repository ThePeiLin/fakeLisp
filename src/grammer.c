				...
				for(size_t i=0;i<len;i++)
				{
					...
					if(sym->isterm)
					{
						if(sym->term_type==FKL_TERM_BUILTIN)
						{
							...
							change|=!fklPutHashItem2(&firstItem->first,&la,NULL);
							if(r)
							{
								if(i==lastIdx)
								{
...
static inline FklHashTable* get_first_set_from_first_sets(const FklGrammer* g
...
static inline void lr1_item_set_closure(FklHashTable* itemSet,FklGrammer* g,FklHashTable* cache)
{
	...
	FklHashTable* processing_set=&pendingSet;
	FklHashTable* next_set=&changeSet;
	while(processing_set->num)
	{
		for(FklHashTableItem* l=processing_set->first;l;l=l->next)
		{
			FklLalrItem* i=(FklLalrItem*)l->data;
			FklGrammerSym* next=get_item_next(i);
			if(next&&!next->isterm)
			{
				...
				int hasEpsilon=0;
				FklHashTable* first=get_la_first_set(g,i->prod,beta,cache,&hasEpsilon);
				...
				if(hasEpsilon)
				{
					for(FklGrammerProduction* prod=prods;prod;prod=prod->next)
					{
						FklLalrItem newItem={.prod=prod,.la=i->la,.idx=0};
						if(!fklPutHashItem2(itemSet,&newItem,NULL))
							fklPutHashItem(&newItem,next_set);
					}
				}
		...
		fklClearHashTable(processing_set);
		FklHashTable* t=processing_set;
	...
	for(FklLookAheadSpreads* sp=itemset->spreads;sp;sp=sp->next)
	{
		...
		for(FklHashTableItem* il=items->first;il;il=il->next)
		{
					...
					&&item->idx==srcItem->idx)
			{
				...
				newItem.la=item->la;
				change|=!fklPutHashItem2(dstItems,&newItem,NULL);
			}
		}
	}
	return change;
...
static inline void init_lalr_look_ahead(FklHashTable* lr0,FklGrammer* g,FklHashTable* cache)
{
...
static inline void add_look_ahead_for_all_item_set(FklHashTable* lr0,FklGrammer* g,FklHashTable* cache)
{
	for(FklHashTableItem* isl=lr0->first;isl;isl=isl->next)
	{
		...
		,FILE* fp)
{
	...
	for(const FklHashTableItem* l=i->first;l;l=l->next)
	{
		...
		idx=c->idx;
		fprintf(fp,"\t\"I%"FKL_PRT64U"\"[fontname=\"Courier\" nojustify=true shape=\"box\" label=\"I%"FKL_PRT64U"\\l\\\n"
				,idx
				,idx);
		...
		for(FklLalrItemSetLink* l=s->links;l;l=l->next)
		{
			...
			fprintf(fp,"\tI%"FKL_PRT64U"->I%"FKL_PRT64U"[fontname=\"Courier\" label=\"",idx,c->idx);
		...
		,FILE* fp)
{
	...
	else
	{
		...
		fprintf(fp,"\",%"FKL_PRT64U",*in+otherMatchLen,*restLen-otherMatchLen))",igs->str->size);
	}
...
static inline void print_get_max_non_term_length_to_c_file(const FklGrammer* g,FILE* fp)
{
	...
	if(g->sortedTerminalsNum)
	{
		...
		fprintf(fp,"\",%"FKL_PRT64U",*in+otherMatchLen,*restLen-otherMatchLen))",cur->size);
		...
		,FILE* fp)
{
	switch(ac->match.t)
	{
		case FKL_LALR_MATCH_STRING_END_WITH_TERMINAL:
			...
			fprintf(fp,"\",%"FKL_PRT64U",*in+otherMatchLen,*restLen-otherMatchLen,outerCtx,start))"
					,ac->match.str->size);
			break;
		case FKL_LALR_MATCH_STRING:
			fputs("(matchLen=fklCharBufMatch(\"",fp);
			print_string_in_hex(ac->match.str,fp);
			fprintf(fp,"\",%"FKL_PRT64U",*in+otherMatchLen,*restLen-otherMatchLen))",ac->match.str->size);
			break;
		case FKL_LALR_MATCH_REGEX:
			{
				uint64_t num=0;
		...
		,FILE* fp)
{
	...
	{
		case FKL_ANALYSIS_SHIFT:
			fprintf(fp,"\t\t\tfklPushPtrStack((void*)state_%"FKL_PRT64U",stateStack);\n",ac->state-states);
		...
		case FKL_ANALYSIS_REDUCE:
			...
			if(ac->prod->len)
				fprintf(fp,"\t\t\tfor(size_t i=0;i<%"FKL_PRT64U";i++)\n"
						"\t\t\t{\n"
			...
			if(ac->prod->len)
			{
				fprintf(fp,"\t\t\tsize_t line=fklGetFirstNthLine(lineStack,%"FKL_PRT64U",outerCtx->line);\n",ac->prod->len);
			...
			if(ac->prod->len)
				fprintf(fp,"\t\t\tfor(size_t i=0;i<%"FKL_PRT64U";i++)\n"
						...
						,ast_destroyer_name);
			fputs("\t\t\tif(!ast)\n\t\t\t{\n\t\t\t\t*errLine=line;\n\t\t\t\treturn FKL_PARSE_REDUCE_FAILED;\n\t\t\t}\n",fp);
			fprintf(fp,"\t\t\tfklPushPtrStack((void*)create_nonterm_analyzing_symbol(%"FKL_PRT64U",ast),symbolStack);\n",ac->prod->left.sid);
...
{
	fprintf(fp,"static int state_%"FKL_PRT64U"(FklPtrStack*"
			",FklPtrStack*"
			",FklUintStack*"
			",int"
			",FklSid_t"
		...
		,FILE* fp)
{
	...
	fprintf(fp,"static int state_%"FKL_PRT64U"(FklPtrStack* stateStack\n"
	...
	if(gt)
	{
		...
		fprintf(fp,"\t\t\t*pfunc=(void*)state_%"FKL_PRT64U";\n\t\t\treturn 0;\n\t\t}\n",gt->state-states);
		gt=gt->next;
		for(;gt;gt=gt->next)
		{
