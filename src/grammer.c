...
{
	return nonterm_hash(k);
}

static inline int nonterm_eq(const FklGrammerNonterm* nt0,const FklGrammerNonterm* nt1)
{
...
{
	FklGrammerProdHashItem* prod=(FklGrammerProdHashItem*)d;
...
{
	.size=sizeof(FklGrammerProdHashItem),
	.__setKey=prod_hash_set_key,
...
#define DEFINE_DEFAULT_C_MATCH_COND(NAME) static void builtin_match_##NAME##_print_c_match_cond(void* ctx,const FklGrammer* g,FILE* fp)\
{\
...
// static void builtin_match_any_print_c_match_cond(void* c,const FklGrammer* g,FILE* fp)
// {
...
static inline const FklRegexCode* add_regex_and_get_terminal(FklRegexTable* t
		,const char* buf
		,size_t len
		,FklSymbolTable* tt)
{
	...
	for(uint32_t i=0;i<st.top;i++)
	{
		...
		switch(*(s->str))
		{
			...
			case '#':
				u->isterm=1;
				u->nt.group=0;
				u->nt.sid=fklAddSymbolCharBuf(&s->str[1],s->size-1,termTable)->id;
				break;
			case '/':
				{
					u->isterm=1;
					u->term_type=FKL_TERM_REGEX;
					const FklRegexCode* re=add_regex_and_get_terminal(regexTable,&s->str[1],s->size-1,termTable);
					if(!re)
					{
			...
			case '&':
				{
					...
					const FklLalrBuiltinMatch* builtin=fklGetBuiltinMatch(builtins,id);
					if(builtin)
					{
						u->isterm=1;
		...
		,FklRegexTable* rt)
{
			...
			case '/':
				{
					u->isterm=1;
					u->term_type=FKL_TERM_REGEX;
					const FklRegexCode* re=add_regex_and_get_terminal(rt,&s->str[1],s->size-1,termTable);
					if(!re)
					{
						fklDestroyGrammerProduction(prod);
						goto error;
	...
	while(!fklIsPtrStackEmpty(&st))
		free(fklPopPtrStack(&st));
	fklUninitPtrStack(&st);
	FklGrammerIgnore* ig=fklGrammerSymbolsToIgnore(prod->syms,prod->len,termTable);
	fklDestroyGrammerProduction(prod);
	return ig;
error:
	while(!fklIsPtrStackEmpty(&st))
...
void fklInitGrammerProductionTable(FklHashTable* s)
{
...
{
	FklGrammerProduction* r=fklCreateEmptyProduction(prod->left.group
			,prod->left.sid
			,prod->len
			,prod->name
			,prod->func
...
int fklAddProdAndExtraToGrammer(FklGrammer* g,FklGrammerProduction* prod)
{
	...
	else
	{
		...
		{
			g->start=*left;
			FklGrammerProduction* extra_prod=create_extra_production(left->group,left->sid);
			extra_prod->next=NULL;
			item=fklPutHashItem(&extra_prod->left,productions);
			item->prods=extra_prod;
			item->prods->idx=g->prodNum;
			g->prodNum++;
		}
		prod->next=NULL;
...
static inline int nonterm_gt(const FklGrammerNonterm* nt0,const FklGrammerNonterm* nt1)
{
	return ((nt0->group<<32)+nt0->sid)>((nt1->group<<32)+nt1->sid);
}

static int nonterm_lt(const FklGrammerNonterm* nt0,const FklGrammerNonterm* nt1)
{
	return ((nt0->group<<32)+nt0->sid)>((nt1->group<<32)+nt1->sid);
}
...
static inline int grammer_sym_equal(const FklGrammerSym* s0,const FklGrammerSym* s1)
{
	return s0->isterm==s1->isterm
		&&s0->term_type==s1->term_type
		&&(s0->term_type==FKL_TERM_BUILTIN
				?builtin_grammer_sym_equal(&s0->b,&s1->b)
...
static inline uintptr_t grammer_sym_hash(const FklGrammerSym* s)
{
	return (s->isterm<<3)
		+(s->term_type==FKL_TERM_BUILTIN
				?builtin_grammer_sym_hash(&s->b)<<2
...
// static void builtin_match_qstr_print_c_match_cond(void* c,const FklGrammer* g,FILE* fp)
// {
// 	fputs("builtin_match_qstr(start,*in+otherMatchLen,*restLen-otherMatchLen,&matchLen,outerCtx,&is_waiting_for_more,",fp);
// 	const FklString* s=c;
// 	fputc('"',fp);
// 	print_string_in_hex(s,fp);
// 	fprintf(fp,"\",%"PRT64U"",s->size);
// 	fputc(')',fp);
// }
...
// static int builtin_match_until_match(void* c
// 		,const char* start
// 		,const char* cstr
// 		,size_t restLen
// 		,size_t* matchLen
...
// {
// 	fputs("static int builtin_match_until(const char* start\n"
// 			"\t\t,const char* cstr\n"
// 			"\t\t,size_t restLen\n"
// 			"\t\t,size_t* matchLen\n"
// 			"\t\t,FklGrammerMatchOuterCtx* outerCtx\n"
...
// static void builtin_match_until_print_c_match_cond(void* c,const FklGrammer* g,FILE* fp)
// {
// 	fputs("builtin_match_qstr(start,*in+otherMatchLen,*restLen-otherMatchLen,&matchLen,outerCtx,&is_waiting_for_more,",fp);
// 	const FklString* s=c;
// 	fputc('"',fp);
...
// 	.ctx_equal=builtin_match_qstr_equal,
// 	.ctx_hash=builtin_match_qstr_hash,
// 	.ctx_create=builtin_match_until_create,
// 	.ctx_destroy=builtin_match_qstr_destroy,
// 	.print_src=builtin_match_until_print_src,
// 	.print_c_match_cond=builtin_match_until_print_c_match_cond,
// };
		...
		,int* is_waiting_for_more)
{
	...
	for(size_t i=0;i<len;i++)
	{
		...
		{
			int last_is_true=0;
			uint32_t len=fklRegexLexMatchp(ig->re
					,str
					,restLen
					,&last_is_true);
			if(len>restLen)
		...
		,int* failed)
{
	...
	if(prod->len==2)
	{
		if(!prod->syms[1].isterm
				||prod->syms[1].term_type!=FKL_TERM_STRING
...
#define DEFINE_LISP_NUMBER_PRINT_C_MATCH_COND(NAME) static void builtin_match_##NAME##_print_c_match_cond(void* c,const FklGrammer* g,FILE* fp)\
{\
	fputs("builtin_match_"#NAME"(start,*in+otherMatchLen,*restLen-otherMatchLen,&matchLen,outerCtx,",fp);\
	...
	{\
		...
		fputc('\"',fp);\
		fprintf(fp,",%"PRT64U"",ctx->start->size);\
	}\
	else\
		fputs("NULL,0",fp);\
	fputc(')',fp);\
...
static void builtin_match_symbol_print_c_match_cond(void* c,const FklGrammer* g,FILE* fp)
{
	MatchSymbolCtx* ctx=c;
	const FklString* start=ctx->start;
	const FklString* end=ctx->end;
	fputs("builtin_match_symbol(start,*in+otherMatchLen,*restLen-otherMatchLen,&matchLen,outerCtx,&is_waiting_for_more,",fp);
	fputc('\"',fp);
	print_string_in_hex(start,fp);
	fprintf(fp,"\",%"PRT64U",\"",start->size);
	print_string_in_hex(end,fp);
	fprintf(fp,"\",%"PRT64U")",end->size);
}
...
// }
//
// static void builtin_match_string_print_c_match_cond(void* c,const FklGrammer* g,FILE* fp)
// {
// 	MatchStringCtx* ctx=c;
// 	const FklString* start=ctx->start;
// 	const FklString* end=ctx->end;
// 	fputs("builtin_match_string(start,*in+otherMatchLen,*restLen-otherMatchLen,&matchLen,outerCtx,&is_waiting_for_more,",fp);
// 	fputc('\"',fp);
// 	print_string_in_hex(start,fp);
// 	fprintf(fp,"\",%"PRT64U",\"",start->size);
// 	print_string_in_hex(end,fp);
...
void fklClearGrammer(FklGrammer* g)
{
	...
	g->start.group=0;
	g->start.sid=0;
	fklClearHashTable(&g->productions);
	fklClearHashTable(&g->firstSets);
...
static inline int init_all_builtin_grammer_sym(FklGrammer* g)
{
	...
	{
		FklGrammerProdHashItem* i=(FklGrammerProdHashItem*)pl->data;
		for(FklGrammerProduction* prod=i->prods;prod;prod=prod->next)
		{
			for(size_t i=prod->len;i>0;i--)
			{
				...
				else if(u->isterm&&u->end_with_terminal&&!g->sortedTerminalsNum)
					sort_reachable_terminals(g);
				if(failed)
					break;
			}
...
int fklAddIgnoreToIgnoreList(FklGrammerIgnore** pp,FklGrammerIgnore* ig)
{
	for(;*pp;pp=&(*pp)->next)
	{
		...
		for(size_t i=0;i<len;i++)
		{
			...
			{
				if(igs0->term_type==FKL_TERM_BUILTIN&&builtin_grammer_sym_equal(&igs0->b,&igs1->b))
					return 1;
				else if(igs0->term_type==FKL_TERM_REGEX&&igs0->re==igs1->re)
					return 1;
...
static inline int is_builtin_terminal_match_epsilon(const FklLalrBuiltinGrammerSym* b)
{
	int is_waiting_for_more=0;
	size_t matchLen=0;
	FklGrammerMatchOuterCtx outerCtx={.maxNonterminalLen=0,.line=0,.start=NULL,.cur=NULL,.create=NULL,.destroy=NULL,};
	return b->t->match(b->c,"","",0,&matchLen,&outerCtx,&is_waiting_for_more);
}
...
static inline int compute_all_first_set(FklGrammer* g)
{
	...
	{
		const FklGrammerProdHashItem* prod=(const FklGrammerProdHashItem*)sidl->data;
		if(is_Sq_nt(&prod->left))
			continue;
		FirstSetItem* item=fklPutHashItem(&prod->left,firsetSets);
		item->hasEpsilon=0;
	...
	do
	{
		...
		{
			const FklGrammerProdHashItem* leftProd=(const FklGrammerProdHashItem*)leftProds->data;
			if(is_Sq_nt(&leftProd->left))
				continue;
...
static inline int add_reachable_terminal(FklGrammer* g)
{
	...
	FklGrammerProdHashItem* first=(FklGrammerProdHashItem*)g->productions.first->data;
	...
	while(!fklIsPtrStackEmpty(&prod_stack))
	{
		...
		for(;prods;prods=prods->next)
		{
			...
			for(size_t i=0;i<prods->len;i++)
			{
				...
				if(cur->isterm)
				{
					if(cur->term_type==FKL_TERM_STRING&&!cur->end_with_terminal)
...
{
	FklHashTable* productions=&g->productions;
	for(const FklHashTableItem* il=productions->first;il;il=il->next)
	{
		const FklGrammerProdHashItem* prod_item=(const FklGrammerProdHashItem*)il->data;
		for(const FklGrammerProduction* prods=prod_item->prods;prods;prods=prods->next)
		{
			const FklGrammerSym* syms=prods->syms;
			for(size_t i=0;i<prods->len;i++)
			{
				const FklGrammerSym* cur=&syms[i];
				if(!cur->isterm&&!fklGetHashItem(&cur->nt,productions))
					return 1;
			}
		}
	}
...
FklGrammer* fklCreateGrammerFromCstr(const char* str[],FklSymbolTable* st)
{
	...
	for(;*str;str++)
	{
		if(**str=='+')
		{
			FklGrammerIgnore* ignore=create_grammer_ignore_from_cstr(*str,builtins,st,tt,rt);
			if(!ignore)
			{
...
int fklAddExtraProdToGrammer(FklGrammer* g)
{
	...
	FklGrammerProduction* extra_prod=create_extra_production(left.group,left.sid);
	extra_prod->next=NULL;
	FklGrammerProdHashItem* item=fklPutHashItem(&extra_prod->left,&g->productions);
	item->prods=extra_prod;
	item->prods->idx=g->prodNum;
	g->prodNum++;
	return 0;
...
static inline uintptr_t lalr_look_ahead_hash_func(const FklLalrItemLookAhead* la)
{
	...
	return rest+((uintptr_t)la->t)+(la->delim<<1)+(la->end_with_terminal);
}
	...
	if(la0->t==la1->t)
	{
		switch(la0->t)
		{
			...
			case FKL_LALR_MATCH_STRING_END_WITH_TERMINAL:
				...
				break;
			case FKL_LALR_MATCH_REGEX:
				return ((int64_t)la0->re->totalsize)-((int64_t)la1->re->totalsize);
				break;
		}
	}
	else
...
static inline void lr0_item_set_closure(FklHashTable* itemSet,FklGrammer* g)
{
	...
	do
	{
		...
		{
			const FklGrammerNonterm* left=((const FklGrammerNonterm*)lefts->data);
			FklGrammerProduction* prod=fklGetGrammerProductions(g,left->group,left->sid);
...
void fklPrintItemSet(const FklHashTable* itemSet,const FklGrammer* g,const FklSymbolTable* st,FILE* fp)
{
	...
	for(FklHashTableItem* list=itemSet->first;list;list=list->next)
	{
		...
		{
			fputs(" / ",fp);
			print_look_ahead(fp,&item->la,&g->regexes);
		}
	}
	putc('\n',fp);
}
...
FklHashTable* fklGenerateLr0Items(FklGrammer* grammer)
{
	...
	const FklGrammerNonterm left={.group=0,.sid=0,};
	FklGrammerProduction* prod=((FklGrammerProdHashItem*)fklGetHashItem(&left,&grammer->productions))->prods;
	FklHashTable items;
...
static inline int get_first_set(const FklGrammer* g
		...
		,FklHashTable* first)
{
	return get_first_set_from_first_sets(g,prod,idx,first);
}
	...
	while(pendingSet.num)
	{
		for(FklHashTableItem* l=pendingSet.first;l;l=l->next)
		{
			...
			if(next&&!next->isterm)
			{
				...
				FklGrammerProduction* prods=fklGetGrammerProductions(g,left->group,left->sid);
...
static inline void print_item_set_as_dot(const FklHashTable* itemSet
		,const FklGrammer* g
		,const FklSymbolTable* st
		,FILE* fp)
{
	...
	for(FklHashTableItem* list=itemSet->first;list;list=list->next)
	{
		...
		{
			fputs(" / ",fp);
			print_look_ahead_as_dot(fp,&item->la,&g->regexes);
		}
	}
	fputs("\\l\\\n",fp);
}
		...
		,FILE* fp)
{
	...
	for(const FklHashTableItem* l=i->first;l;l=l->next)
	{
		...
		idx=c->idx;
		fprintf(fp,"\t\"I%"PRT64U"\"[fontname=\"Courier\" nojustify=true shape=\"box\" label=\"I%"PRT64U"\\l\\\n"
				,idx
				,idx);
		...
		for(FklLalrItemSetLink* l=s->links;l;l=l->next)
		{
			...
			fprintf(fp,"\tI%"PRT64U"->I%"PRT64U"[fontname=\"Courier\" label=\"",idx,c->idx);
		...
		,FklAnalysisState* state)
{
	...
	{
		const FklString* s=fklGetSymbolWithId(sym->nt.sid,tt)->symbol;
		action->match.t=sym->end_with_terminal?FKL_LALR_MATCH_STRING_END_WITH_TERMINAL:FKL_LALR_MATCH_STRING;
		action->match.str=s;
	}
	return action;
}
		...
		,const FklLalrItemLookAhead* la)
{
	...
	else if(action->match.t==FKL_LALR_MATCH_STRING_END_WITH_TERMINAL)
	{
		...
		for(;*pa;pa=&(*pa)->next)
		{
			FklAnalysisStateAction* curAction=*pa;
			if(curAction->match.t!=FKL_LALR_MATCH_STRING_END_WITH_TERMINAL
					||(curAction->match.t==FKL_LALR_MATCH_STRING_END_WITH_TERMINAL
						&&s->size>curAction->match.str->size))
				break;
		}
		action->next=*pa;
		...
		,int* is_waiting_for_more)
{
	...
	return ignore_match(ig,start,str,restLen,pmatchLen,outerCtx,is_waiting_for_more);
}
		...
		,FILE* fp)
{
	...
	else
	{
		...
		fprintf(fp,"\",%"PRT64U",*in+otherMatchLen,*restLen-otherMatchLen))",igs->str->size);
	}
...
static inline FklAnalysisStateAction* create_builtin_ignore_action(FklGrammer* g,FklGrammerIgnore* ig)
{
	...
	if(ig->len==1)
	{
		...
		action->match.t=igs->term_type==FKL_TERM_BUILTIN
			?FKL_LALR_MATCH_BUILTIN
			:igs->term_type==FKL_TERM_REGEX
			...
			switch(actions->action)
			{
				case FKL_ANALYSIS_SHIFT:
					...
					print_look_ahead_of_analysis_table(fp,&actions->match);
					{
						...
						fprintf(fp," , %"PRT64U" )",idx);
					}
					break;
				case FKL_ANALYSIS_REDUCE:
					fputs("R(",fp);
					print_look_ahead_of_analysis_table(fp,&actions->match);
					fprintf(fp," , %"PRT64U" )",actions->prod->idx);
					break;
...
static inline FklAnalysisStateGoto* find_gt(FklAnalysisStateGoto* gt,FklSid_t group,FklSid_t id)
{
	for(;gt;gt=gt->next)
	{
		...
		,FILE* fp)
{
	...
	for(size_t i=0;i<num;i++)
	{
					...
					case FKL_ANALYSIS_SHIFT:
						{
							...
							fprintf(fp,"s%"PRT64U"",idx);
						}
						break;
					case FKL_ANALYSIS_REDUCE:
						fprintf(fp,"r%"PRT64U"",action->prod->idx);
						break;
					case FKL_ANALYSIS_ACCEPT:
						fputs("acc",fp);
						break;
		...
		for(FklHashTableItem* sl=sidList;sl;sl=sl->next)
		{
			fputc('|',fp);
			const FklGrammerNonterm* id=(const FklGrammerNonterm*)sl->data;
			FklAnalysisStateGoto* gt=find_gt(curState->state.gt,id->group,id->sid);
			if(gt)
			{
				uintptr_t idx=gt->state-states;
				fprintf(fp,"%"PRT64U"",idx);
...
static inline void print_get_max_non_term_length_to_c_file(const FklGrammer* g,FILE* fp)
{
	...
	if(g->sortedTerminalsNum)
	{
		...
		fprintf(fp,"\",%"PRT64U",*in+otherMatchLen,*restLen-otherMatchLen))",cur->size);
...
static inline void print_match_char_buf_end_with_terminal_prototype_to_c_file(FILE* fp)
{
...
static inline void print_match_char_buf_end_with_terminal_to_c_file(const FklGrammer* g,FILE* fp)
{
	fputs("static inline size_t match_char_buf_end_with_terminal(const char* pattern\n"
			"\t\t,size_t pattern_size\n"
			"\t\t,const char* cstr\n"
			"\t\t,size_t restLen\n"
			"\t\t,FklGrammerMatchOuterCtx* outerCtx\n"
			...
			"\treturn maxNonterminalLen==pattern_size?fklCharBufMatch(pattern,pattern_size,cstr,restLen):0;\n"
			"}\n"
			,fp);
}

static inline void print_state_action_match_to_c_file(const FklAnalysisStateAction* ac
		,const FklGrammer* g
		,FILE* fp)
{
	switch(ac->match.t)
	{
		case FKL_LALR_MATCH_STRING_END_WITH_TERMINAL:
			...
			fprintf(fp,"\",%"PRT64U",*in+otherMatchLen,*restLen-otherMatchLen,outerCtx,start))"
					,ac->match.str->size);
			break;
		case FKL_LALR_MATCH_STRING:
			fputs("(matchLen=fklCharBufMatch(\"",fp);
			print_string_in_hex(ac->match.str,fp);
			fprintf(fp,"\",%"PRT64U",*in+otherMatchLen,*restLen-otherMatchLen))",ac->match.str->size);
			break;
		case FKL_LALR_MATCH_REGEX:
			{
...
static inline void print_state_action_to_c_file(const FklAnalysisStateAction* ac
		...
		,FILE* fp)
{
	...
	{
		case FKL_ANALYSIS_SHIFT:
			fprintf(fp,"\t\t\tfklPushPtrStack((void*)state_%"PRT64U",stateStack);\n",ac->state-states);
			fputs("\t\t\tfklPushPtrStack(create_term_analyzing_symbol(*in,matchLen,outerCtx->line,outerCtx->ctx),symbolStack);\n"
		...
		case FKL_ANALYSIS_ACCEPT:
			...
			break;
		case FKL_ANALYSIS_REDUCE:
			fprintf(fp,"\t\t\tstateStack->top-=%"PRT64U";\n",ac->prod->len);
			fprintf(fp,"\t\t\tsymbolStack->top-=%"PRT64U";\n",ac->prod->len);
			fputs("\t\t\tFklStateFuncPtr func=(FklStateFuncPtr)fklTopPtrStack(stateStack);\n",fp);
			fputs("\t\t\tFklStateFuncPtr nextState=NULL;\n",fp);
			fprintf(fp,"\t\t\tfunc(NULL,NULL,NULL,0,%"PRT64U",(void**)&nextState,NULL,NULL,NULL,NULL,NULL,NULL,NULL);\n",ac->prod->left.sid);
			fputs("\t\t\tif(!nextState)\n\t\t\t\treturn FKL_PARSE_REDUCE_FAILED;\n",fp);
			fputs("\t\t\tfklPushPtrStack((void*)nextState,stateStack);\n",fp);

			if(ac->prod->len)
				fprintf(fp,"\t\t\tvoid** nodes=(void**)malloc(sizeof(void*)*%"PRT64U");\n"
						"\t\t\tFKL_ASSERT(nodes);\n"
			...
			if(ac->prod->len)
				fprintf(fp,"\t\t\tfor(size_t i=0;i<%"PRT64U";i++)\n"
						"\t\t\t{\n"
			...
			if(ac->prod->len)
			{
				fprintf(fp,"\t\t\tsize_t line=fklGetFirstNthLine(lineStack,%"PRT64U",outerCtx->line);\n",ac->prod->len);
				fprintf(fp,"\t\t\tlineStack->top-=%"PRT64U";\n",ac->prod->len);
			}
			else
				fputs("\t\t\tsize_t line=outerCtx->line;\n",fp);

			fprintf(fp,"\t\t\tvoid* ast=%s(outerCtx->ctx,nodes,%"PRT64U",line,st);\n"
					,ac->prod->name
					,ac->prod->len);
			...
			if(ac->prod->len)
				fprintf(fp,"\t\t\tfor(size_t i=0;i<%"PRT64U";i++)\n"
						...
						,ast_destroyer_name);
			fputs("\t\t\tif(!ast)\n\t\t\t{\n\t\t\t\t*errLine=line;\n\t\t\t\treturn FKL_PARSE_REDUCE_FAILED;\n\t\t\t}\n",fp);
			fprintf(fp,"\t\t\tfklPushPtrStack((void*)create_nonterm_analyzing_symbol(%"PRT64U",ast),symbolStack);\n",ac->prod->left.sid);
...
{
	fprintf(fp,"static int state_%"PRT64U"(FklPtrStack*"
			",FklPtrStack*"
			",FklUintStack*"
			",int"
			",FklSid_t"
		...
		,FILE* fp)
{
	...
	fprintf(fp,"static int state_%"PRT64U"(FklPtrStack* stateStack\n"
	...
	if(ac)
	{
		...
		fputs("\t\telse\n\t\t\treturn is_waiting_for_more?FKL_PARSE_WAITING_FOR_MORE:FKL_PARSE_TERMINAL_MATCH_FAILED;\n",fp);
	}
	else
	...
	if(gt)
	{
		fprintf(fp,"\t\tif(left==%"PRT64U")\n\t\t{\n",gt->nt.sid);
		fprintf(fp,"\t\t\t*pfunc=(void*)state_%"PRT64U";\n\t\t\treturn 0;\n\t\t}\n",gt->state-states);
		gt=gt->next;
		for(;gt;gt=gt->next)
		{
			fprintf(fp,"\t\telse if(left==%"PRT64U")\n\t\t{\n",gt->nt.sid);
			fprintf(fp,"\t\t\t*pfunc=(void*)state_%"PRT64U";\n\t\t\treturn 0;\n\t\t}\n",gt->state-states);
		}
		fputs("\t\telse\n\t\t\treturn FKL_PARSE_REDUCE_FAILED;\n",fp);
	}
...
{
	fputs("static inline int regex_lex_match_for_parser_in_c(const FklRegexCode* re,const char* cstr,size_t restLen,size_t* matchLen,int* is_waiting_for_more)\n"
			"{\n"
			"\tint last_is_true=0;\n"
...
#undef BUFFER_SIZE
	...
	fprintf(fp,"void %s(FklPtrStack* stateStack){fklPushPtrStack((void*)state_0,stateStack);}\n\n",state_0_push_func_name);
}
...
{
	SidBuiltinHashItem* i=fklGetHashItem(&id,ht);
	if(i)
		return i->t;
	return NULL;
}

int fklIsNonterminalExist(FklHashTable* prods,FklSid_t group_id,FklSid_t sid)
{
	FklGrammerNonterm left={.group=group_id,.sid=sid};
...
static inline int match_char_buf_end_with_terminal(const FklString* laString
		...
		,const char* start)
{
	size_t maxNonterminalLen=get_max_non_term_length(g,outerctx,start,cstr,restLen);
		...
		,const FklGrammer* g)
{
	switch(match->t)
	{
		...
		case FKL_LALR_MATCH_STRING_END_WITH_TERMINAL:
			{
				const FklString* laString=match->str;
				if(match_char_buf_end_with_terminal(laString,cstr,restLen,g,outerCtx,start))
				{
					*matchLen=laString->size;
					return 1;
				}
		...
		case FKL_LALR_MATCH_EOF:
			*matchLen=1;
			...
			break;
		case FKL_LALR_MATCH_BUILTIN:
			if(match->func.t->match(match->func.c,start,cstr,restLen,matchLen,outerCtx,is_waiting_for_more))
				return 1;
			break;
		case FKL_LALR_MATCH_REGEX:
			{
...
static inline void dbg_print_state_stack(FklPtrStack* stateStack,FklAnalysisState* states)
{
	for(size_t i=0;i<stateStack->top;i++)
	{
		...
		fprintf(stderr,"%"PRT64U" ",curState-states);
	}
	fputc('\n',stderr);
}
		...
		,size_t* errLine)
{
	...
	fklPushPtrStack(create_nonterm_analyzing_symbol(left.group,left.sid,ast),symbolStack);
	fklPushUintStack(line,lineStack);
	fklPushPtrStack((void*)state,stateStack);
	return 0;
}

void* fklParseWithTableForCstrDbg(const FklGrammer* g
		,const char* cstr
		,FklGrammerMatchOuterCtx* outerCtx
		,FklSymbolTable* st
		,int* err)
