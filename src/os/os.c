...
static int os_rename(FKL_CPROC_ARGL) {
    ...
    FKL_CPROC_RETURN(
        exe,ctx, FKL_MAKE_VM_FIX(rename(old_name_str->str, new_name_str->str)));
    return 0;
}
...
static int os_getenv(FKL_CPROC_ARGL) {
    ...
    // FKL_VM_PUSH_VALUE(exe, env_var ? fklCreateVMvalueStrFromCstr(exe, env_var)
    //                                : FKL_VM_NIL);
    FKL_CPROC_RETURN(exe,ctx, env_var ? fklCreateVMvalueStrFromCstr(exe, env_var)
                                   : FKL_VM_NIL);
    return 0;
}
...
static int os_setenv(FKL_CPROC_ARGL) {
    ...
    FklVMvalue* name=FKL_CPROC_GET_ARG(exe,ctx,0);
    FklVMvalue* value=arg_num>1
        ?FKL_CPROC_GET_ARG(exe,ctx,1)
        :NULL;
    FklVMvalue* overwrite=arg_num>2
