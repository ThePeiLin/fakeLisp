		...
		else
		{
			...
			while(num<UINT8_MAX)
			{
				if(&pat[len]>=end
					...
					break;
				if(pat[len]=='\\')
					len+=count_number_esc_char(&pat[len+1],end-&pat[len+1]);
				num++;
				len++;
			}
			retval+=2+num;
...
static inline uint32_t char_buf_to_char_class(const char* pat
		,size_t len
		,uint8_t* patrns)
{
	const char* end=&pat[len];
	...
	while(pat<end)
	{
		...
		else
		{
			...
			for(uint32_t i=0;i<len;i++)
			{
				if(pat[i]=='\\')
				{
					uint32_t esc_len=count_number_esc_char(&pat[i+1],len-i-1);
...
static inline int compile_count(struct ReCompileCtx* ctx
		,const char* pat
		,size_t len)
{
	size_t i=0;
	...
	while(i<len)
	{
		...
		else if(c=='(')
		{
			if(i+1>=len)
				return 1;
			brcnt++;
			struct ReInst* inst=(struct ReInst*)fklRealloc(ctx->inst,(ctx->ecnt+1)*sizeof(struct ReInst));
			FKL_ASSERT(inst);
			ctx->inst=inst;
		...
		switch(c)
		{
			...
			case '\\':
				...
				else
				{
					cur_obj->trueoffset=j+1;
					cur_obj->falseoffset=inst[resel].failoff;
					uint32_t esc_len=count_number_esc_char(&pattern[i+1],len-i-1);
					switch(pattern[i+1])
					{
						case 'd':
...
void fklRegexFree(FklRegexCode* re)
{
	free(re);
}
...
static inline int matchone(const FklRegexObj* cur,const uint8_t* patrns,uint8_t c)
{
	switch(cur->type)
	{
		case FKL_REGEX_DOT:
...
#define STP state[sp]
#define POP() sp--
#define PUSH() state[sp+1].offset=state[sp].offset;\
...
uint32_t fklRegexMatchpInCstr(const FklRegexCode* re
		,const char* str
		,uint32_t* ppos)
{
...
static inline void print_objs(const FklRegexObj* obj,uint32_t objs_num,FILE* fp)
{
...
void fklRegexPrintAsC(const FklRegexCode* re
		,const char* prefix
		,const char* pattern
...
static void str_regex_hash_set_val(void* d0,const void* d1)
{
	*(FklStrRegexHashItem*)d0=*(const FklStrRegexHashItem*)d1;
}

static int str_regex_hash_key_equal(const void* k0,const void* k1)
{
	return fklStringEqual(*(const FklString**)k0,*(const FklString**)k1);
}

static uintptr_t str_regex_hash_func(const void* k0)
{
	return fklStringHash(*(const FklString**)k0);
}

static void str_regex_uninit(void* d)
{
	FklStrRegexHashItem* item=(FklStrRegexHashItem*)d;
	free(item->str);
	fklRegexFree(item->re);
}

static int str_regex_other_hash_key_equal(const void* k0,const void* k1)
{
	const CharBufKey* k=(const CharBufKey*)k1;
	return !fklStringCharBufCmp(*(const FklString**)k0,k->len,k->buf);
}

static void str_regex_other_hash_set_val(void* d0,const void* d1)
{
	FklStrRegexHashItem* n=(FklStrRegexHashItem*)d0;
	const CharBufKey* k=(const CharBufKey*)d1;
	n->str=fklCreateString(k->len,k->buf);
	n->re=NULL;
}

static uintptr_t str_regex_other_hash_func(const void* k0)
{
	const CharBufKey* k=(const CharBufKey*)k0;
	return fklCharBufHash(k->buf,k->len);
}

static const FklHashTableMetaTable str_regex_hash_meta_table=
{
	.size=sizeof(FklStrRegexHashItem),
	.__getKey=fklHashDefaultGetKey,
	.__setKey=fklPtrKeySet,
	.__setVal=str_regex_hash_set_val,
	.__keyEqual=str_regex_hash_key_equal,
	.__hashFunc=str_regex_hash_func,
...
const FklRegexCode* fklAddRegexCharBuf(FklRegexTable* t
		,const char* buf
		,size_t len)
{
	FklHashTable* ht=&t->str_re;
	CharBufKey key={.len=len,.buf=buf,};
	FklStrRegexHashItem* item=fklGetOrPutWithOtherKey(&key
			,str_regex_other_hash_func
			,str_regex_other_hash_key_equal
			,str_regex_other_hash_set_val
			,ht);
	if(item->re)
		return item->re;
