...
void SYS_eqn(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if((FKL_IS_DBL(fir)||FKL_IS_I32(fir)||FKL_IS_I64(fir))&&(FKL_IS_DBL(sec)||FKL_IS_I32(sec)||FKL_IS_I64(sec)))
		FKL_SET_RETURN("SYS_eqn",((((FKL_IS_DBL(fir))?fir->u.dbl
							:((FKL_IS_I32(fir)?FKL_GET_I32(fir)
								:fir->u.i64)))
						-((FKL_IS_DBL(sec))?sec->u.dbl
...
void SYS_add(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	{
		rd+=r64;
		FKL_SET_RETURN("SYS_add",fklNewVMvalue(FKL_DBL,&rd,exe->heap),stack);
	}
	else
	{
...
void SYS_add_1(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(!FKL_IS_DBL(arg)&&!FKL_IS_I32(arg)&&!FKL_IS_I64(arg))
		FKL_RAISE_BUILTIN_ERROR("sys.add_1",FKL_WRONGARG,runnable,exe);
	if(FKL_IS_DBL(arg))
	{
		double r=arg->u.dbl+1.0;
		FKL_SET_RETURN("SYS_add_1",fklNewVMvalue(FKL_DBL,&r,exe->heap),stack);
	}
...
void SYS_sub(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(!FKL_IS_DBL(prev)&&!FKL_IS_I32(prev)&&!FKL_IS_I64(prev))
	...
	if(!cur)
	{
		...
		{
			rd=-prev->u.dbl;
			FKL_SET_RETURN("SYS_sub",fklNewVMvalue(FKL_DBL,&rd,exe->heap),stack);
		}
	...
	else
	{
		...
		fklResBp(stack);
		if(FKL_IS_DBL(prev)||rd!=0.0)
		{
			rd=((FKL_IS_DBL(prev))?prev->u.dbl:((FKL_IS_I64(prev))?prev->u.i64:FKL_GET_I32(prev)))-rd-r64;
			FKL_SET_RETURN("SYS_sub",fklNewVMvalue(FKL_DBL,&rd,exe->heap),stack);
		}
		else
		{
...
void SYS_sub_1(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	{
		double r=arg->u.dbl-1.0;
		FKL_SET_RETURN("SYS_sub_1",fklNewVMvalue(FKL_DBL,&r,exe->heap),stack);
	}
	else if(FKL_IS_I64(arg))
	{
		int64_t r=arg->u.i64-1;
...
void SYS_mul(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	{
		rd*=r64;
		FKL_SET_RETURN("SYS_mul",fklNewVMvalue(FKL_DBL,&rd,exe->heap),stack);
	}
	else
	{
		if(r64>INT32_MAX||r64<INT32_MIN)
...
void SYS_div(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(!cur)
	{
		...
		else if(FKL_IS_I64(prev))
		{
			...
			{
				rd=1.0/r64;
				FKL_SET_RETURN("SYS_div",fklNewVMvalue(FKL_DBL,&rd,exe->heap),stack);
			}
			else
			{
				r64=1/r64;
	...
	else
	{
		...
		if((r64)==0)
			FKL_RAISE_BUILTIN_ERROR("sys.div",FKL_DIVZERROERROR,runnable,exe);
		fklResBp(stack);
		if(FKL_IS_DBL(prev)||rd!=1.0||(FKL_IS_I32(prev)&&FKL_GET_I32(prev)%(r64))||(FKL_IS_I64(prev)&&prev->u.i64%(r64)))
		{
			if(rd==0.0||r64==0)
				FKL_RAISE_BUILTIN_ERROR("sys.div",FKL_DIVZERROERROR,runnable,exe);
			rd=((double)((FKL_IS_DBL(prev))?prev->u.dbl:(FKL_IS_I32(prev)?FKL_GET_I32(prev):prev->u.i64)))/rd/r64;
			FKL_SET_RETURN("SYS_div",fklNewVMvalue(FKL_DBL,&rd,exe->heap),stack);
		}
		else
		{
...
void SYS_rem(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(FKL_IS_DBL(fir)||FKL_IS_DBL(sec))
	{
		double af=(FKL_IS_DBL(fir))?fir->u.dbl:(FKL_IS_I32(fir)?FKL_GET_I32(fir):fir->u.i64);
		double as=(FKL_IS_DBL(sec))?sec->u.dbl:(FKL_IS_I32(sec)?FKL_GET_I32(sec):sec->u.i64);
		if(as==0.0)
			FKL_RAISE_BUILTIN_ERROR("sys.rem",FKL_DIVZERROERROR,runnable,exe);
		double r=fmod(af,as);
		FKL_SET_RETURN("SYS_rem",fklNewVMvalue(FKL_DBL,&r,exe->heap),stack);
	}
...
void SYS_gt(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	for(;cur;cur=fklGET_VAL(fklPopVMstack(stack),heap))
	{
		...
		{
			if((FKL_IS_DBL(prev)||FKL_IS_I32(prev)||FKL_IS_I64(prev))
					&&(FKL_IS_DBL(cur)||FKL_IS_I32(cur)||FKL_IS_I64(cur)))
				r=((((FKL_IS_DBL(prev))?prev->u.dbl
								:((FKL_IS_I32(prev))?FKL_GET_I32(prev)
									:prev->u.i64))
							-((FKL_IS_DBL(cur))?cur->u.dbl
...
void SYS_chr(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	FklVMvalue* obj=fklGET_VAL(fklPopVMstack(stack),exe->heap);
	if(fklResBp(stack))
		FKL_RAISE_BUILTIN_ERROR("sys.chr",FKL_TOOMANYARG,runnable,exe);
	if(!obj)
		FKL_RAISE_BUILTIN_ERROR("sys.chr",FKL_TOOFEWARG,runnable,exe);
	if(FKL_IS_I32(obj))
		FKL_SET_RETURN("SYS_chr",FKL_MAKE_VM_CHR(FKL_GET_I32(obj)),stack);
	else if(FKL_IS_CHR(obj))
		FKL_SET_RETURN("SYS_chr",obj,stack);
	else if(FKL_IS_DBL(obj))
		FKL_SET_RETURN("SYS_chr",FKL_MAKE_VM_CHR((int32_t)obj->u.dbl),stack);
	else if(FKL_IS_STR(obj))
		FKL_SET_RETURN("SYS_chr",FKL_MAKE_VM_CHR(obj->u.str[0]),stack);
	else if(FKL_IS_BYTS(obj))
		FKL_SET_RETURN("SYS_chr",FKL_MAKE_VM_CHR(obj->u.byts->str[0]),stack);
	else
		FKL_RAISE_BUILTIN_ERROR("sys.chr",FKL_WRONGARG,runnable,exe);
}
...
void SYS_int(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(FKL_IS_CHR(obj))
		FKL_SET_RETURN("SYS_int",FKL_MAKE_VM_I32(FKL_GET_CHR(obj)),stack);
...
void SYS_dbl(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	FklVMvalue* obj=fklGET_VAL(fklPopVMstack(stack),exe->heap);
	if(fklResBp(stack))
		FKL_RAISE_BUILTIN_ERROR("sys.dbl",FKL_TOOMANYARG,runnable,exe);
	if(!obj)
		FKL_RAISE_BUILTIN_ERROR("sys.dbl",FKL_TOOFEWARG,runnable,exe);
	FklVMvalue* retval=fklNewVMvalue(FKL_DBL,NULL,exe->heap);
	if(FKL_IS_CHR(obj))
		retval->u.dbl=(double)FKL_GET_CHR(obj);
	else if(FKL_IS_I32(obj))
		retval->u.dbl=(double)FKL_GET_I32(obj);
	else if(FKL_IS_DBL(obj))
	...
	else if(FKL_IS_BYTS(obj))
	{
		...
		FKL_RAISE_BUILTIN_ERROR("sys.dbl",FKL_WRONGARG,runnable,exe);
	FKL_SET_RETURN("SYS_dbl",retval,stack);
}
...
void SYS_sym(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(FKL_IS_CHR(obj))
		FKL_SET_RETURN("SYS_sym",FKL_MAKE_VM_SYM(fklAddSymbolToGlob(((char[]){FKL_GET_CHR(obj),'\0'}))->id),stack);
	else if(FKL_IS_SYM(obj))
...
void SYS_byts(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	else if(FKL_IS_CHR(obj))
	{
		...
		*(char*)retval->u.byts->str=FKL_GET_CHR(obj);
	}
	else if(FKL_IS_SYM(obj))
	{
...
void SYS_go(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(!FKL_IS_PRC(threadProc)&&!FKL_IS_DLPROC(threadProc)&&!FKL_IS_CONT(threadProc)&&!FKL_IS_FLPROC(threadProc))
		FKL_RAISE_BUILTIN_ERROR("sys.go",FKL_WRONGARG,runnable,exe);
	FklVM* threadVM=(FKL_IS_PRC(threadProc)||FKL_IS_CONT(threadProc))?fklNewThreadVM(threadProc->u.prc,exe->heap):fklNewThreadDlprocVM(runnable,exe->heap);
...
void SYS_clcc(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(!FKL_IS_PTR(proc)||(proc->type!=FKL_PRC&&proc->type!=FKL_CONT&&proc->type!=FKL_DLPROC))
...
void SYS_apply(FklVM* exe,pthread_rwlock_t* gclock)
{
	...
	if(!FKL_IS_PTR(proc)||(proc->type!=FKL_PRC&&proc->type!=FKL_CONT&&proc->type!=FKL_DLPROC&&proc->type!=FKL_FLPROC))
