...
static void inline B_import(FklVM* exe,FklVMframe* frame)
{
	...
	{
		char* cstr=NULL;
		uint32_t scope=fklGetU32FromByteCode(fklGetCompoundFrameCodeAndAdd(frame,sizeof(scope)));
		process_import(plib,exe,frame,&cstr,scope);
		if(cstr)
...
inline static FklVMvalue* volatile* get_var_ref(FklVMframe* frame,uint32_t idx,FklPrototypePool* ptpool,FklSid_t* psid)
{
	...
	if(v==FKL_VM_NIL)
	{
		...
		FklVMvalue* ref=find_ref(ptpool->pts,proc,def);
		if(ref!=FKL_VM_NIL)
		{
			proc->unresolveRef--;
			lr->ref[idx]=ref;
