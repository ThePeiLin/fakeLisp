...
static inline void insert_local_ref(FklVMCompoundFrameVarRef* lr,FklVMvarRef* ref)
{
}
	...
	if(count)
	{
		...
		for(uint32_t i=0;i<count;i++)
		{
			...
			if(c->isLocal)
			{
				...
				{
					FklVMvarRef* ref=fklCreateVMvarRef(lr->loc,c->cidx);
					closure[i]=ref;
					insert_local_ref(lr,ref);
				}
			}
			else
			{
