...
#define DO_TRAPPING_STEP_VM(exe) {\
	FklVMframe* curframe=exe->top_frame;\
	switch(curframe->type)\
	{\
		case FKL_FRAME_COMPOUND:\
...
static void B_notice_lock_call(FKL_VM_INS_FUNC_ARGL)
{
	...
	if(!fklIsCallable(proc))
		FKL_RAISE_BUILTIN_ERROR_CSTR(FKL_ERR_CALL_ERROR,exe);
	switch(proc->type)
	{
		case FKL_TYPE_PROC:
...
{
	if(exe->tp==exe->bp)
		FKL_RAISE_BUILTIN_ERROR_CSTR(FKL_ERR_STACKERROR,exe);
...
static inline void B_load_dll(FKL_VM_INS_FUNC_ARGL)
{
	...
	if(!plib->imported)
	{
		...
		if(dll)
			...
			FKL_RAISE_BUILTIN_INVALIDSYMBOL_ERROR_CSTR(errorStr?errorStr:realpath->str,errorStr!=NULL,FKL_ERR_IMPORTFAILED,exe);
		if(!initFunc)
			FKL_RAISE_BUILTIN_INVALIDSYMBOL_ERROR_CSTR(realpath->str,0,FKL_ERR_IMPORTFAILED,exe);
		uint32_t tp=exe->tp;
...
static inline void B_get_var_ref(FKL_VM_INS_FUNC_ARGL)
{
	...
	if(id)
	{
		...
		FKL_RAISE_BUILTIN_INVALIDSYMBOL_ERROR_CSTR(str->str,0,FKL_ERR_SYMUNDEFINE,exe);
	}
		...
		,FklVMlib* libs)
{
	...
	memcpy(exe->ins_table,prev->ins_table,sizeof(InsFuncTable));
	uv_mutex_init(&exe->lock);
