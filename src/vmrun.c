...
start:
    ...
    case FKL_OP_IMPORT_XX:
        ...
        frame->c.pc += 2;
        import : { FklVMlib *plib = exe->importingLib;
...
load_dll: {
    if (!plib->imported) {
        ...
        FklVMvalue *dll =
            fklCreateVMvalueDll(exe, FKL_VM_STR(plib->proc)->str, &errorStr);
        ...
        if (!initFunc)
            FKL_RAISE_BUILTIN_ERROR_FMT(FKL_ERR_IMPORTFAILED, exe,
                                        "Failed to import dll: %s", plib->proc);
        uint32_t tp = exe->tp;
...
case FKL_OP_GT3: {
    ...
    int err = 0;
    int r =
        fklVMvalueCmp(a, b, &err) > 0 && !err && fklVMvalueCmp(b, c, &err) > 0;
    if (err)
        FKL_RAISE_BUILTIN_ERROR(FKL_ERR_INCORRECT_TYPE_VALUE, exe);
    FKL_VM_PUSH_VALUE(exe, r ? FKL_VM_TRUE : FKL_VM_NIL);
} break;
